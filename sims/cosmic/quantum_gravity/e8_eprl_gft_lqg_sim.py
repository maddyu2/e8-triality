...
# LQG EPRL/GFT proxies (spin j ~0-5/2, intertwiner dim ~1-10, foam complexity ~3-6 valence)
# Vary spin_j / intertwiner_dim / foam_complexity as input vectors
spin_j = torch.linspace(0.0, 2.5, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
intertwiner_dim = torch.linspace(1, 10, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
foam_complexity = torch.linspace(3, 6, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

# Quantum symmetry proxy (~0.85–1.0)
qg_sym = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

real_lqg_data = torch.cat([spin_j, intertwiner_dim, foam_complexity, qg_sym], dim=-1).repeat(1, 1, dim // 4) * torch.randn(batch_size, seq_len, dim, device=device) * 0.01
...
# Sectors: Spin j, Intertwiner dim, Foam complexity, QG symmetry, Prediction nulling
spin_roots = e8_roots[:60]
inter_roots = e8_roots[60:120]
foam_roots = e8_roots[120:180]
sym_roots = e8_roots[180:]

class LQGSimRotary(nn.Module):
    def __init__(self):
        super().__init__()
