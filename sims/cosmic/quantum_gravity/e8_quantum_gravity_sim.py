...
# 2025 quantum gravity proxies (Planck length ~1e-35 m, G ~6.67e-11, horizon entropy S = A/4)
# Vary planck_length / grav_constant / horizon_entropy as input vectors
planck_length = torch.linspace(1e-36, 1e-34, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # m
grav_constant = torch.linspace(6e-11, 7e-11, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # m³ kg⁻¹ s⁻²
horizon_entropy = torch.linspace(1e3, 1e5, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # Planck units

# Gravity symmetry proxy (~0.85–1.0)
grav_sym = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

real_qg_data = torch.cat([planck_length, grav_constant, horizon_entropy, grav_sym], dim=-1).repeat(1, 1, dim // 4) * torch.randn(batch_size, seq_len, dim, device=device) * 0.01
...
# Initial QG state → precision target
states = real_qg_data
target_prec = torch.ones(batch_size, 1, device=device)

model = E8QuantumGravity().to(device)
...
plt.tight_layout()
plt.savefig("qg_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as qg_precision_entropy.png")
