...
# LQG spin network proxies (spin labels j = 0–5/2, edge volume ~√j(j+1), node valence 3–6)
# Vary spin_label / edge_volume / node_valence as input vectors
spin_label = torch.linspace(0.0, 2.5, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # j (half-integer)
edge_volume = torch.linspace(0.0, 5.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)   # √j(j+1) proxy
node_valence = torch.linspace(3.0, 6.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)   # typical 4-valent

# Spin network symmetry proxy (SU(2) invariance factor ~0.85–1.0)
spin_sym = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

real_lqg_data = torch.cat([spin_label, edge_volume, node_valence, spin_sym], dim=-1)\
...
# Sectors: Spin label, Edge volume, Node valence, Spin symmetry, Prediction nulling
spin_roots   = e8_roots[:48]
volume_roots = e8_roots[48:96]
valence_roots = e8_roots[96:144]
sym_roots    = e8_roots[144:192]
pred_roots   = e8_roots[192:]

class SpinNetRotary(nn.Module):
    def __init__(self):
        super().__init__()
...
class E8SpinNetworksLQG(nn.Module):
...
# Initial spin network state → precision target
states = real_lqg_data
target_prec = torch.ones(batch_size, 1, device=device)

model = E8SpinNetworksLQG().to(device)
...
plt.tight_layout()
plt.savefig("spin_networks_lqg_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as spin_networks_lqg_precision_entropy.png")
