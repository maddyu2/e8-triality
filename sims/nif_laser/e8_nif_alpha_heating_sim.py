...
# NIF alpha heating proxies (alpha power ~1–3 MJ, deposition efficiency >70%, burn fraction 0.5–1.0)
alpha_power = torch.linspace(1, 3, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # MJ
deposition_eff = torch.linspace(0.7, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
burn_frac = torch.linspace(0.5, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

heating_sym = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

real_data = torch.cat([alpha_power, deposition_eff, burn_frac, heating_sym], dim=-1)\
...
# Sectors: Alpha power, Deposition eff, Burn frac, Heating symmetry, Prediction nulling
power_roots = e8_roots[:60]
eff_roots = e8_roots[60:120]
burn_roots = e8_roots[120:180]
...
class E8NIFAlphaHeating(nn.Module):
...
model = E8NIFAlphaHeating().to(device)
...
class AblationModel(E8NIFAlphaHeating):
    def __init__(self, depth=256):
...
plt.tight_layout()
plt.savefig("nif_alpha_heating_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as nif_alpha_heating_ablation_precision_entropy.png")
