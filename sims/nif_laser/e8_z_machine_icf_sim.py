...
# Z machine ICF proxies (pulse energy ~20–30 MJ, yield Q ~0.1–0.5, implosion velocity ~100–200 km/s)
pulse_energy = torch.linspace(20, 30, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # MJ
yield_q = torch.linspace(0.1, 0.5, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
implosion_vel = torch.linspace(100, 200, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # km/s

icf_sym = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

real_data = torch.cat([pulse_energy, yield_q, implosion_vel, icf_sym], dim=-1)\
...
# Sectors: Pulse energy, Yield Q, Implosion vel, ICF symmetry, Prediction nulling
pulse_roots = e8_roots[:60]
yield_roots = e8_roots[60:120]
vel_roots = e8_roots[120:180]
sym_roots = e8_roots[180:]

class ZMachineICFRotary(nn.Module):
    def __init__(self):
        super().__init__()
...
# Initial Z machine state → precision target
states = real_data
target_prec = torch.ones(batch_size, 1, device=device)

model = E8ZMachineICF().to(device)
...
plt.tight_layout()
plt.savefig("z_machine_icf_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as z_machine_icf_precision_entropy.png")
