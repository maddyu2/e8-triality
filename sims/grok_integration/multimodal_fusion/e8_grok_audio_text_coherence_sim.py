...
dim = 384
latent_dim = 8
seq_len = 512  # shared time steps (audio samples / text tokens)
batch_size = 64
...
checkpoint_path = os.path.join(checkpoint_dir, "audio_text_coherence_checkpoint.pth")

# Synthetic Grok audio-text coherence proxy (audio waveforms + text token features + noise/occlusion)
features_audio = 192  # waveform
features_text = 192   # token embedding

audio_text_data = []
for b in range(batch_size):
    t = torch.linspace(0, 10*math.pi, seq_len, device=device)
    
    # Aligned audio coherence (speech-like waves)
    audio = torch.sin(t.unsqueeze(-1) * torch.arange(features_audio, device=device)) * 0.5
    
    # Aligned text tokens (correlated "transcription")
    text = torch.cos(t.unsqueeze(-1) * torch.arange(features_text, device=device) * 1.1) * 0.4
    
    # Stack aligned pair
    pair = torch.cat([audio, text], dim=-1)
    pair += torch.randn_like(pair) * 0.1  # noise
    audio_text_data.append(pair)

audio_text_data = torch.stack(audio_text_data).to(device)

# Project to shared dim
proj = nn.Linear(features_audio + features_text, dim).to(device)
clean_data = proj(audio_text_data)

# High masking (70–90% — partial coherence proxy)
...
class AudioTextCycleBlock(nn.Module):
    def __init__(self):
        super().__init__()
...
class E8AudioTextCoherence(nn.Module):
    def __init__(self, depth=32, use_triality=True):
        super().__init__()
        self.use_triality = use_triality
        self.cycle = AudioTextCycleBlock() if use_triality else DummyCycle()
...
# Models
model = E8AudioTextCoherence(use_triality=True).to(device)
model_ablation = E8AudioTextCoherence(use_triality=False).to(device)
