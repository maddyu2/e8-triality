...
checkpoint_path = os.path.join(checkpoint_dir, "multimodal_occlusion_checkpoint.pth")

# Synthetic Grok multimodal occlusion recovery proxy (video + audio + text features + high occlusion)
features_video = 128
features_audio = 64
features_text = 64
...
# Very high occlusion (80–95% — recovery proxy)
missing_rate = torch.linspace(0.8, 0.95, batch_size, device=device).view(batch_size, 1, 1)
...
class OcclusionCycleBlock(nn.Module):
    def __init__(self):
        super().__init__()
...
class E8OcclusionRecovery(nn.Module):
    def __init__(self, depth=32, use_triality=True):
        super().__init__()
        self.use_triality = use_triality
        self.cycle = OcclusionCycleBlock() if use_triality else DummyCycle()
...
# Models
model = E8OcclusionRecovery(use_triality=True).to(device)
model_ablation = E8OcclusionRecovery(use_triality=False).to(device)
