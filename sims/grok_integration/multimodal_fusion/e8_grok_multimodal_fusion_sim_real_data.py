...
!pip install torch matplotlib numpy datasets

# Second cell: The sim code (optimized — real audio streaming + synthetic video/text, loss curves)
import torch
...
checkpoint_path = os.path.join(checkpoint_dir, "multimodal_fusion_real_checkpoint.pth")

# Real data streaming: LibriSpeech audio (proxy for speech) + synthetic video/text for alignment
# Streaming = low memory, no full download
librispeech = load_dataset("librispeech_asr", "clean", split="train", streaming=True)

# Take small batch for demo (real audio waveforms)
audio_batch = []
for sample in librispeech.take(batch_size):
    audio = torch.tensor(sample["audio"]["array"], device=device).float()
    # Resample/pad to seq_len
    if audio.shape[0] > seq_len:
        audio = audio[:seq_len]
    else:
        audio = F.pad(audio, (0, seq_len - audio.shape[0]))
    audio_batch.append(audio)

audio_data = torch.stack(audio_batch)

# Synthetic video + text aligned to audio length
t = torch.linspace(0, 10*math.pi, seq_len, device=device)
video = torch.sin(t.unsqueeze(-1) * torch.arange(128, device=device)) * 0.5  # motion features
text = torch.cos(t.unsqueeze(-1) * torch.arange(64, device=device) * 1.2) * 0.4  # "caption" features

# Repeat synthetic to batch
video = video.unsqueeze(0).repeat(batch_size, 1, 1)
text = text.unsqueeze(0).repeat(batch_size, 1, 1)

# Stack multimodal (audio real, video/text synthetic proxy)
multimodal_data = torch.cat([video, audio_data.unsqueeze(-1).repeat(1, 1, 64), text], dim=-1)

# Project to shared dim
proj = nn.Linear(128 + 64 + 64, dim).to(device)
...
plt.title('E8 Triality vs Ablation Loss Curves — Multimodal Fusion (Real Audio Streaming + Synthetic Proxy)')
plt.legend()
plt.grid(True)
plt.show()
