...
# Synthetic FSD sensor fusion proxy (lidar + camera + radar features + noise/occlusion)
...
for b in range(batch_size):
    ...
    lidar = torch.sin(t.unsqueeze(-1) * torch.arange(features_per_sensor, device=device)) * 0.5
    camera = torch.cos(t.unsqueeze(-1) * torch.arange(features_per_sensor, device=device)) * 0.4
    radar = torch.sin(t.unsqueeze(-1) * torch.arange(features_per_sensor, device=device)) * 0.3
    
    frame = torch.cat([lidar, camera, radar], dim=-1)
...
# Apply masking (40–70% missing — sensor dropout/dust/fog)
...
# Models
model = E8FSD Fusion(use_triality=True).to(device)
model_ablation = E8FSD Fusion(use_triality=False).to(device)
