...
dim = 384
latent_dim = 8
seq_len = 1024  # time steps (sensor frames)
batch_size = 64
epochs = 20000
lr = 5e-5
use_amp = True
...
# Synthetic Wayve multimodal fusion proxy (camera vision + lidar + radar features + noise/occlusion)
features_per_sensor = 64
sensors = 3  # camera, lidar, radar

sensor_data = []
for b in range(batch_size):
    t = torch.linspace(0, 10*math.pi, seq_len, device=device)
    camera = torch.cos(t.unsqueeze(-1) * torch.arange(features_per_sensor, device=device)) * 0.5  # vision patterns
    lidar = torch.sin(t.unsqueeze(-1) * torch.arange(features_per_sensor, device=device) * 1.2) * 0.4  # depth/motion
    radar = torch.sin(t.unsqueeze(-1) * torch.arange(features_per_sensor, device=device) * 1.5) * 0.3  # velocity
    
    frame = torch.cat([camera, lidar, radar], dim=-1)
    frame += torch.randn_like(frame) * 0.1  # noise
    sensor_data.append(frame)

sensor_data = torch.stack(sensor_data).to(device)  # (batch, seq_len, features)

# Project to dim
proj = nn.Linear(features_per_sensor * sensors, dim).to(device)
clean_data = proj(sensor_data)

# Apply masking (40–70% missing readings — sensor dropout/fog/rain)
...
class WayveCycleBlock(nn.Module):
    def __init__(self):
        super().__init__()
...
class E8WayveFusion(nn.Module):
    def __init__(self, depth=32, use_triality=True):
        super().__init__()
        self.use_triality = use_triality
        self.cycle = WayveCycleBlock() if use_triality else DummyCycle()
...
# Models
model = E8WayveFusion(use_triality=True).to(device)
model_ablation = E8WayveFusion(use_triality=False).to(device)
