...
from torch.optim.lr_schedule import CosineAnnealingLR
import torch.amp
...
class RWKVCycleBlock(nn.Module):
    def __init__(self):
        super().__init__()
...
# RWKV-style time-mixing + channel-mixing layer
...
# Model with RWKV + triality
class E8RWKVLongContext(nn.Module):
    def __init__(self, depth=256, use_triality=True):
        super().__init__()
        self.use_triality = use_triality
        self.layers = nn.ModuleList([RWKVLayer() for _ in range(depth)])
        self.cycle_block = RWKVCycleBlock()
...
# Training – optimized
# ────────────────────────────────────────────────
model = E8RWKVLongContext(use_triality=True).to(device)
...
# ────────────────────────────────────────────────
model_ablation = E8RWKVLongContext(use_triality=False).to(device)
...
plt.tight_layout()
plt.savefig("rwkv_triality_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as rwkv_triality_ablation_precision_entropy.png")
