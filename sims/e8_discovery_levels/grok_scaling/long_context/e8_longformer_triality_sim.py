...
# ────────────────────────────────────────────────
# CONFIG – optimized with Longformer-style sliding window
# ────────────────────────────────────────────────
device = 'cuda' if torch.cuda.is_available() else 'cpu'
triality = 3
dim = 240
latent_dim = 8
seq_len = 1024  # proxy — sliding window scales effective context to 10k–20k+
window_size = 256  # Longformer sliding window size
global_tokens = 8   # number of global tokens (proxy)
noise_scale = 0.002
batch_size = 256
micro_batch_size = 64
...
class LongformerCycleBlock(nn.Module):
    def __init__(self):
        super().__init__()
...
# Longformer-style sliding window attention (simplified)
class LongformerAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # Sliding window mask (proxy — full Longformer has dilated + global)
        attn_mask = torch.ones(b, s, s, device=device)
        for i in range(s):
            start = max(0, i - window_size // 2)
            end = min(s, i + window_size // 2)
            attn_mask[:, i, start:end] = 0  # attend to window

        # Global tokens (first global_tokens attend to all)
        attn_mask[:, :global_tokens, :] = 0
        attn_mask[:, :, :global_tokens] = 0

        attn_output = F.scaled_dot_product_attention(q, k, v, attn_mask=attn_mask.bool(), is_causal=False)
        x = self.out_proj(attn_output)
        return x

# Model with Longformer attention + triality
class E8LongformerTriality(nn.Module):
    def __init__(self, depth=256, use_triality=True):
        super().__init__()
        self.use_triality = use_triality
        self.heads = triality if use_triality else 1
        self.layers = nn.ModuleList([
            LongformerAttention(heads=self.heads)
...
# Training – optimized
# ────────────────────────────────────────────────
model = E8LongformerTriality(use_triality=True).to(device)
...
# ────────────────────────────────────────────────
model_ablation = E8LongformerTriality(use_triality=False).to(device)
...
plt.tight_layout()
plt.savefig("longformer_triality_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as longformer_triality_ablation_precision_entropy.png")
