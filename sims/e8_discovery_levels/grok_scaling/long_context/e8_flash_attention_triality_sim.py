...
import torch.nn.functional as F  # for scaled_dot_product_attention (FlashAttention fused kernel)
...
# ────────────────────────────────────────────────
# CONFIG – optimized with FlashAttention fused kernel
# ────────────────────────────────────────────────
device = 'cuda' if torch.cuda.is_available() else 'cpu'
triality = 3
dim = 240
latent_dim = 8
seq_len = 1024  # proxy — fused kernel scales to 10k–20k+ efficiently
noise_scale = 0.002
batch_size = 256
micro_batch_size = 64
...
class FlashCycleBlock(nn.Module):
    def __init__(self):
        super().__init__()
...
# Model with FlashAttention fused kernel + triality
class E8FlashAttentionTriality(nn.Module):
    def __init__(self, depth=256, use_triality=True):
        ...
        self.cycle_block = FlashCycleBlock()
    ...
    def forward(self, x, step):
        ...
        q = q.view(x.shape[0], x.shape[1], self.heads, self.head_dim)
        k = k.view(x.shape[0], x.shape[1], self.heads, self.head_dim)
        v = v.view(x.shape[0], x.shape[1], self.heads, self.head_dim)
...
# Training – optimized
# ────────────────────────────────────────────────
model = E8FlashAttentionTriality(use_triality=True).to(device)
...
# ────────────────────────────────────────────────
model_ablation = E8FlashAttentionTriality(use_triality=False).to(device)
...
plt.tight_layout()
plt.savefig("flash_attention_triality_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as flash_attention_triality_ablation_precision_entropy.png")
