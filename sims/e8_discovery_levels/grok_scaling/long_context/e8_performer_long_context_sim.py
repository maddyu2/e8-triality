...
# ────────────────────────────────────────────────
# CONFIG – optimized with Performer-style FAVOR+ attention
# ────────────────────────────────────────────────
device = 'cuda' if torch.cuda.is_available() else 'cpu'
triality = 3
dim = 240
latent_dim = 8
seq_len = 1024  # proxy — Performer scales to 10k–20k+ efficiently
noise_scale = 0.002
batch_size = 256
micro_batch_size = 64
...
lr = 5e-5
warmup_steps = 2000
num_features = 256  # random features for FAVOR+ (Performer approximation)
...
class PerformerCycleBlock(nn.Module):
    def __init__(self):
        super().__init__()
...
# Performer-style FAVOR+ attention (linear approximation)
class PerformerAttention(nn.Module):
    def __init__(self, heads=triality):
        super().__init__()
        self.heads = heads
        ...
        self.random_features = nn.Parameter(torch.randn(self.heads, self.head_dim, self.num_features), requires_grad=False)
        self.random_features = self.random_features / self.random_features.norm(dim=-1, keepdim=True)
    ...
    def forward(self, x):
        ...
        # FAVOR+ approximation
        q_prime = torch.einsum('bshd,hdf->bshf', q, self.random_features)
        k_prime = torch.einsum('bshd,hdf->bshf', k, self.random_features)

        q_prime = torch.exp(q_prime)
        k_prime = torch.exp(k_prime)

        # Numerator and denominator
        kv = torch.einsum('bshf,bshd->bhfd', k_prime, v)
        denom = k_prime.sum(dim=1, keepdim=True)

        attn_output = torch.einsum('bshf,bhfd->bshd', q_prime, kv) / (q_prime @ denom.transpose(-2, -1))

        attn_output = attn_output.view(b, s, d)
        x = self.out_proj(attn_output)
        return x

# Model with Performer attention + triality
class E8PerformerLongContext(nn.Module):
    def __init__(self, depth=256, use_triality=True):
        super().__init__()
        self.use_triality = use_triality
        self.heads = triality if use_triality else 1
        self.layers = nn.ModuleList([
            PerformerAttention(heads=self.heads)
            for _ in range(depth)
        ])
        self.cycle_block = PerformerCycleBlock()
    ...
    def forward(self, x, step):
        ...
        for layer in self.layers:
            if use_checkpoint:
                x = checkpoint(lambda l, x: l(x), layer, x, use_reentrant=False)
            else:
                x = layer(x)
            x = x + self.norm(x)
...
# Training – optimized
# ────────────────────────────────────────────────
model = E8PerformerLongContext(use_triality=True).to(device)
...
# ────────────────────────────────────────────────
model_ablation = E8PerformerLongContext(use_triality=False).to(device)
...
plt.tight_layout()
plt.savefig("performer_long_context_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as performer_long_context_ablation_precision_entropy.png")
