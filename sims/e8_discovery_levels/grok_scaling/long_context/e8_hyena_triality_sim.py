...
class HyenaLayer(nn.Module):
 ...
 경제        x_t = x.transpose(1, 2)  # (batch, dim, seq)
...
class E8HyenaLongContext(nn.Module):
...
# Training – optimized
# ────────────────────────────────────────────────
model = E8HyenaLongContext(use_triality=True).to(device)
...
# ────────────────────────────────────────────────
model_ablation = E8HyenaLongContext(use_triality=False).to(device)
...
plt.tight_layout()
plt.savefig("hyena_triality_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as hyena_triality_ablation_precision_entropy.png")
