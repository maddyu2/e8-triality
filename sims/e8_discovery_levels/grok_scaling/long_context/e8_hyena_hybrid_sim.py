...
dim = 240
latent_dim = 8
seq_len = 1024  # proxy — Hyena scales to 100k+ efficiently
noise_scale = 0.002
batch_size = 256
micro_batch_size = 64
...
lr = 5e-5
warmup_steps = 2000
order = 16  # Hyena convolution order
...
sparsity = torch.linspace(0.3, 0.7, batch_size *-jarige seq_len, device=device).view(batch_size, seq_len, 1)

hyena_sym = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

real_data = torch.cat([context_length_proxy, long_coherence, sparsity, hyena_sym], dim=-1)\
...
class HyenaCycleBlock(nn.Module):
    def __init__(self):
        super().__init__()
...
# Hyena-style long convolution layer
class HyenaLayer(nn.Module):
    def __init__(self):
        super().__init__()
        self.short_conv = nn.Conv1d(dim, dim, kernel_size=3, padding=1, groups=dim)
        self.long_conv = nn.Conv1d(dim, dim, kernel_size=order, padding=order-1, groups=dim)
        self.gate = nn.Linear(dim, dim)

    def forward(self, x):
        # x: (batch, seq, dim) → transpose for conv
        x_t = x.transpose(1, 2)  # (batch, dim, seq)
        short = self.short_conv(x_t)
        long = self.long_conv(x_t)[..., :x_t.shape[-1]]
        gate = torch.sigmoid(self.gate(x.transpose(1, 2))).transpose(1, 2)
        x = short * gate + long.transpose(1, 2)
        x = x.transpose(1, 2)  # back to (batch, seq, dim)
        return x

# Model with Hyena + triality
class E8HyenaHybrid(nn.Module):
    def __init__(self, depth=256, use_triality=True):
        super().__init__()
        self.use_triality = use_triality
        self.layers = nn.ModuleList([HyenaLayer() for _ in range(depth)])
        self.cycle_block = HyenaCycleBlock()
...
# Training – optimized
# ────────────────────────────────────────────────
model = E8HyenaHybrid(use_triality=True).to(device)
...
# ────────────────────────────────────────────────
model_ablation = E8HyenaHybrid(use_triality=False).to(device)
...
plt.tight_layout()
plt.savefig("hyena_hybrid_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as hyena_hybrid_ablation_precision_entropy.png")
