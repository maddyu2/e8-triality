...
# Mamba-style selective SSM (simplified for demo — real Mamba uses CUDA kernels)
class MambaLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        # Selective SSM (discretized — simplified recurrence)
        A = -torch.exp(self.A_log)  # (d, state_size)
        h = torch.zeros(b, d, state_size, device=device)
        out = []
        for t in range(s):
            h = h * A + x[:, t:t+1] * x[:, t:t+1]  # simplified (real Mamba uses selective scan)
...
class E8MambaStateSpace(nn.Module):
...
# Training – optimized
# ────────────────────────────────────────────────
model = E8MambaStateSpace(use_triality=True).to(device)
...
# ────────────────────────────────────────────────
model_ablation = E8MambaStateSpace(use_triality=False).to(device)
...
plt.tight_layout()
plt.savefig("mamba_state_space_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as mamba_state_space_ablation_precision_entropy.png")
