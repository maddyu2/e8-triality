...
# ────────────────────────────────────────────────
# CONFIG – optimized for Pro A100 (long-context reasoning proxy)
# ────────────────────────────────────────────────
triality = 3
dim = 240
latent_dim = 8
seq_len = 16384  # 16k token proxy (long-horizon reasoning chain)
batch_size = 64
epochs = 50000  # large for Pro — full convergence
lr = 5e-5
use_amp = True

# Long-context reasoning chain proxy (chained dependencies + noise)
chain = torch.linspace(0.1, 0.9, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
real_data = chain.repeat(1, 1, dim) * torch.randn(batch_size, seq_len, dim, device=device) * 0.002

# Apply masking (40–70% missing tokens — sparse reasoning)
...
target = chain.repeat(1, 1, dim)  # coherent chain target
...
class E8LongContextReasoning(nn.Module):
    def __init__(self, depth=64, use_triality=True):
        super().__init__()
        self.use_triality = use_triality
        self.cycle = LongContextCycleBlock() if use_triality else nn.Identity()
...
model = E8LongContextReasoning(use_triality=True).to(device)
...
model_ablation = E8LongContextReasoning(use_triality=False).to(device)
...
plt.title("Precision Convergence (16k Token Reasoning Chain)")
...
plt.title("Entropy Convergence (16k Token Reasoning Chain)")
plt.xlabel("Epoch / 500")
plt.ylabel("Entropy")
plt.legend()
plt.grid(True)
...
plt.tight_layout()
plt.savefig("long_context_reasoning_sigma_visualization.png")
plt.show()

print("Sigma visualization saved as long_context_reasoning_sigma_visualization.png")