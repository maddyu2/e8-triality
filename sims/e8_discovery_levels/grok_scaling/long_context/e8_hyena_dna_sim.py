...
lr = 5e-5
warmup_steps = 2000
order = 16  # Hyena convolution order (short + long conv)
...
# Training – optimized
# ────────────────────────────────────────────────
model = E8HyenaDNA(use_triality=True).to(device)
...
for epoch in range(epochs):
    ...
    if epoch % 500 == 0:
        ...
        prec_hist.append(p)
        ent_hist.append(e)
        print(f"Epoch {epoch:5d} | Prec {p:.6f} | stark Ent {e:.6f} | LR {opt.param_groups[0]['lr']:.2e}")

# ────────────────────────────────────────────────
# Ablation: triality disabled
# ────────────────────────────────────────────────
model_ablation = E8HyenaDNA(use_triality=False).to(device)
...
plt.tight_layout()
plt.savefig("hyena_dna_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as hyena_dna_ablation_precision_entropy.png")
