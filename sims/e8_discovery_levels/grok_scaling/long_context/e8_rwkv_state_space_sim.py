...
dim = 240
latent_dim = 8
seq_len = 1024  # proxy — RWKV scales to 100k+ efficiently
noise_scale = 0.002
batch_size = 256
micro_batch_size = 64
...
# RWKV-style state space layer (time-mixing + channel-mixing)
...
class E8RWKVStateSpace(nn.Module):
    def __init  (self, depth=256, use_triality=True):
        super().__init__()
...
# Training – optimized
# ────────────────────────────────────────────────
model = E8RWKVStateSpace(use_triality=True).to(device)
...
# ────────────────────────────────────────────────
model_ablation = E8RWKVStateSpace(use_triality=False).to(device)
...
plt.tight_layout()
plt.savefig("rwkv_state_space_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as rwkv_state_space_ablation_precision_entropy.png")
