...
dim = 240
latent_dim = 8
seq_len = 1024  # proxy — Jamba scales to 100k+ efficiently
noise_scale = 0.002
batch_size = 256
micro_batch_size = 64
...
lr = 5e-5
warmup_steps = 2000
state_size = 16  # Mamba state size (small for demo)
...
jamba_sym = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

real_data = torch.cat([context_length_proxy, long_coherence, sparsity, jamba_sym], dim=-1)\
...
class JambaCycleBlock(nn.Module):
    def __init__(self):
        super().__init__()
...
# Mamba-style SSM layer (simplified)
class MambaLayer(nn.Module):
    def __init__(self):
        super().__init__()
        self.in_proj = nn.Linear(dim, dim * 2, bias=False)
        self.out_proj = nn.Linear(dim, dim, bias=False)
        self.A_log = nn.Parameter(torch.log(torch.arange(1, state_size + 1, device=device).repeat(dim, 1)))
    ...
    def forward(self, x):
        ...
        h = torch.zeros(b, d, state_size, device=device)
        out = []
        for t in range(s):
            h = h * A + x[:, t:t+1] * x[:, t:t+1]  # simplified recurrence
...
# Transformer attention layer (for hybrid)
class TransformerLayer(nn.Module):
    def __init__(self):
        super().__init__()
        self.attn = nn.MultiheadAttention(dim, triality, batch_first=True)
...
# Jamba-style hybrid layer (Mamba + Transformer alternation)
class JambaHybridLayer(nn.Module):
    def __init__(self):
        super().__init__()
...
# Model with Jamba hybrid + triality
class E8JambaHybrid(nn.Module):
    def __init__(self, depth=256, use_triality=True):
        super().__init__()
        self.use_triality = use_triality
        self.layers = nn.ModuleList([JambaHybridLayer() for _ in range(depth)])
        self.cycle_block = JambaCycleBlock()
        self.norm = nn.LayerNorm(dim)
...
# Training – optimized
# ────────────────────────────────────────────────
model = E8JambaHybrid(use_triality=True).to(device)
...
# ────────────────────────────────────────────────
model_ablation = E8JambaHybrid(use_triality=False).to(device)
...
plt.tight_layout()
plt.savefig("jamba_hybrid_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as jamba_hybrid_ablation_precision_entropy.png")
