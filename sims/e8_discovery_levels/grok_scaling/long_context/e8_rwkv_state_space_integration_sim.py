...
# ────────────────────────────────────────────────
# CONFIG – optimized for longer sequences
# ────────────────────────────────────────────────
device = 'cuda' if torch.cuda.is_available() else 'cpu'
triality = 3
dim = 240
latent_dim = 8
seq_len = 2048  # increased proxy for longer sequences (scales to 100k+ on GPU)
noise_scale = 0.002
batch_size = 256
micro_batch_size = 64
...
# ────────────────────────────────────────────────
# Long-context proxies (optimized for RWKV recurrence)
# ────────────────────────────────────────────────
...
rwkv_sym = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

real_data = torch.cat([context_length_proxy, long_coherence, sparsity, rwkv_sym], dim=-1)\
...
class RWKVStateCycleBlock(nn.Module):
    def __init__(self):
        super().__init__()
...
# RWKV-style state space layer
class RWKVStateLayer(nn.Module):
    def __init__(self):
        super().__init__()
        self.time_mix_key = nn.Linear(dim, dim, bias=False)
        self.time_mix_value = nn.Linear(dim, dim, bias=False)
        self.time_mix_receptance = nn.Linear(dim, dim, bias=False)
        self.channel_mix_key = nn.Linear(dim, dim, bias=False)
        self.channel_mix_value = nn.Linear(dim, dim, bias=False)
        self.channel_mix_receptance = nn.Linear(dim, dim, bias=False)

    def forward(self, x, state):
        # Time mixing (RWKV recurrence)
        k = self.time_mix_key(x)
        v = self.time_mix_value(x)
        r = self.time_mix_receptance(x)
        wkv = state + k * v
        time_out = r * wkv

        # Channel mixing
        k = self.channel_mix_key(time_out)
        v = self.channel_mix_value(time_out)
        r = self.channel_mix_receptance(time_out)
        channel_out = r * (k * v)

        new_state = wkv
        return channel_out, new_state

# Model with RWKV state space + triality
class E8RWKVStateSpaceIntegration(nn.Module):
    def __init__(self, depth=256, use_triality=True):
        super().__init__()
        self.use_triality = use_triality
        self.layers = nn.ModuleList([RWKVStateLayer() for _ in range(depth)])
        self.cycle_block = RWKVStateCycleBlock()
        self.norm = nn.LayerNorm(dim)
        self.head = nn.Linear(dim, 1)

    def forward(self, x, step, state=None):
        if state is None:
            state = torch.zeros(x.size(0), dim, device=device)
        ...
        for layer in self.layers:
            if use_checkpoint:
                x, new_state = checkpoint(layer, x, state, use_reentrant=False)
            else:
        ...
        x = self.norm(x)
        return torch.sigmoid(self.head(x.mean(dim=1))), new_state  # return last state for recurrence

# ────────────────────────────────────────────────
# Training – optimized
# ────────────────────────────────────────────────
model = E8RWKVStateSpaceIntegration(use_triality=True).to(device)
...
for epoch in range(epochs):
    ...
    for micro_step in range(grad_accum_steps):
        with torch.amp.autocast(device_type='cuda', dtype=torch.float16) if use_amp else nullcontext():
            prec, state = model(real_data, epoch, state)
...
# ────────────────────────────────────────────────
model_ablation = E8RWKVStateSpaceIntegration(use_triality=False).to(device)
...
for epoch in range(epochs):
    ...
    for micro_step in range(grad_accum_steps):
        with torch.amp.autocast(device_type='cuda', dtype=torch.float16) if use_amp else nullcontext():
            abl_prec, state_ablation = model_ablation(real_data, epoch, state_ablation)
...
plt.tight_layout()
plt.savefig("rwkv_state_space_integration_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as rwkv_state_space_integration_ablation_precision_entropy.png")
