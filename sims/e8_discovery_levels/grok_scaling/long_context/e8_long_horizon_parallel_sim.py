...
dim = 240
latent_dim = 8
seq_len = 1024  # proxy — represents chunks; effective context scales to 10k+ via parallel cycle
noise_scale = 0.002
batch_size = 256
micro_batch_size = 64
...
# ────────────────────────────────────────────────
# Long-horizon proxies (10k+ token coherence, sparsity 0.3–0.7)
# ────────────────────────────────────────────────
context_length_proxy = torch, torch.linspace(10000, 20000, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
...
# Triality Parallel Cycle Block (non-autoregressive)
class LongHorizonCycleBlock(nn.Module):
    ...
    def forward(self, x, step):
        ...
        fused = (x_rot1 + x_rot2 + x_rot3) / triality  # parallel fusion
        return fused

# Model with parallel triality cycle + standard attention (for ablation comparison)
class E8LongHorizonParallel(nn.Module):
    def __init__(self, depth=256, use_triality=True, use_causal=True):
        super().__init__()
        self.use_triality = use_triality
        self.use_causal = use_causal  # causal for autoregressive ablation
        self.heads = triality if use_triality else 8
        self.layers = nn.ModuleList([
            nn.MultiheadAttention(dim, self.heads, batch_first=True, dropout=0.0)
            for _ in range(depth)
        ])
        self.cycle_block = LongHorizonCycleBlock()
    ...
    def forward(self, x, step):
        ...
        for layer in self.layers:
            if use_checkpoint:
                attn_out, _ = checkpoint(lambda l, x: l(x, x, x, need_weights=False, attn_mask=None if not self.use_causal else torch.triu(torch.ones(x.shape[1], x.shape[1]), diagonal=1).bool().to(device)), layer, x, use_reentrant=False)
            else:
                attn_mask = None if not self.use_causal else torch.triu(torch.ones(x.shape[1], x.shape[1]), diagonal=1).bool().to(device)
                attn_out, _ = layer(x, x, x, need_weights=False, attn_mask=attn_mask)
...
# Training – optimized
# ────────────────────────────────────────────────
model = E8LongHorizonParallel(use_triality=True, use_causal=False).to(device)  # parallel (non-causal)
...
# ────────────────────────────────────────────────
# Ablation: autoregressive (causal) attention
# ────────────────────────────────────────────────
model_ablation = E8LongHorizonParallel(use_triality=False, use_causal=True).to(device)  # sequential autoregressive
...
plt.subplot(1,2,1)
plt.plot(prec_hist, label='E8 Triality (Parallel)')
plt.plot(abl_prec_hist, label='Sequential Ablation', linestyle='--')
...
plt.subplot(1,2,2)
plt.plot(ent_hist, label='E8 Triality (Parallel)')
plt.plot(abl_ent_hist, label='Sequential Ablation', linestyle='--')
...
plt.tight_layout()
plt.savefig("long_horizon_parallel_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as long_horizon_parallel_ablation_precision_entropy.png")
