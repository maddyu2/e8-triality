...
dim = 240
latent_dim = 8
seq_len = 1024  # proxy — Mamba scales to 100k+ efficiently
noise_scale = 0.002
batch_size = 256
micro_batch_size = 64
...
mamba_sym = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

real_data = torch.cat([context_length_proxy, long_coherence, sparsity, mamba_sym], dim=-1)\
...
class MambaCycleBlock(nn.Module):
    def __init__(self):
        super().__init__()
...
# Mamba-style selective SSM (simplified)
class MambaLayer(nn.Module):
    ...
    def forward(self, x):
        b, s, d = x.shape
        x_and_z = self.in_proj(x)  # (b, s, 2d)
        x, z = x_and_z.chunk(2, dim=-1)

        # Selective SSM (discretized)
        A = -torch.exp(self.A_log)  # (d, state_size)
        x_dbl = x.unfold(2, state_size, 1)  # approximate discretization
        # Simplified recurrence (for demo — real Mamba uses CUDA kernels)
        h = torch.zeros(b, d, state_size, device=device)
        out = []
        for t in range(s):
            h = h * A + x[:, t:t+1] * x_dbl[:, t] if t > 0 else x[:, t:t+1] * x_dbl[:, t]
...
# Model with Mamba + triality
class E8MambaLongContext(nn.Module):
    def __init__(self, depth=256, use_triality=True):
        super().__init__()
        self.use_triality = use_triality
        self.layers = nn.ModuleList([MambaLayer() for _ in range(depth)])
        self.cycle_block = MambaCycleBlock()
...
# Training – optimized
# ────────────────────────────────────────────────
model = E8MambaLongContext(use_triality=True).to(device)
...
# ────────────────────────────────────────────────
model_ablation = E8MambaLongContext(use_triality=False).to(device)
...
plt.tight_layout()
plt.savefig("mamba_triality_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as mamba_triality_ablation_precision_entropy.png")
