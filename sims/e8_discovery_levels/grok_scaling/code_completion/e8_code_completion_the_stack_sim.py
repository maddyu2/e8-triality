...
dim = 240
latent_dim = 8
seq_len = 512  # shorter for Stack tokens (adjustable)
noise_scale = 0.002
batch_size = 64  # smaller for code data
micro_batch_size = 16
grad_accum_steps = batch_size // micro_batch_size
epochs = 100000  # small for testing — scale up on cloud
use_amp = True
use_checkpoint = True
lr = 5e-5
warmup_steps = 1000
...
# ────────────────────────────────────────────────
# Real The Stack loader (Python subset, 1% for demo)
# ────────────────────────────────────────────────
tokenizer = AutoTokenizer.from_pretrained("bigcode/starcoderbase-1b")  # or "gpt2" for faster
dataset = load_dataset("bigcode/the-stack", data_dir="data/python", split="train[:1%]")  # small subset

def tokenize_batch(batch):
    return tokenizer(batch["content"], truncation=True, max_length=seq_len, padding="max_length", return_tensors="pt")

# Get tokenized batch (proxy for full loader — use DataLoader in production)
tokenized = tokenize_batch(dataset[:batch_size])
input_ids = tokenized["input_ids"].to(device)  # (batch, seq_len)

# Convert to embedding space (simple proxy — use real embedder on better hardware)
real_data = nn.Embedding(tokenizer.vocab_size, dim)(input_ids) * noise_scale

# Apply sparse masking (40–70% missing tokens)
...
# Coherence target (next-token proxy — simplified to ones for reconstruction)
...
class E8CodeCompletion(nn.Module):
    def __init__(self, depth=128, use_triality=True):
        super().__init__()
        self.use_triality = use_triality  # reduced depth for laptop/cloud
...
for epoch in range(epochs):
    opt_op_ablation.zero_grad(set_to_none=True)
...
plt.tight_layout()
plt.savefig("code_completion_the_stack_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as code_completion_the_stack_ablation_precision_entropy.png")
