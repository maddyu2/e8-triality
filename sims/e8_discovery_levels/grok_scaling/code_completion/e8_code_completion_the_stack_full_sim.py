...
dim = 240
latent_dim = 8
seq_len = 512  # shorter for code tokens
noise_scale = 0.002
batch_size = 128
micro_batch_size = 32
grad_accum_steps = batch_size // micro_batch_size
epochs = 50000  # small for testing — scale up on cloud
use_amp = True
use_checkpoint = True
lr = 5e-5
warmup_steps = 1000
...
# ────────────────────────────────────────────────
# Real The Stack loader (Python subset, 10% for validation — full is too large for most hardware)
# ────────────────────────────────────────────────
tokenizer = AutoTokenizer.from_pretrained("bigcode/starcoderbase-1b")
dataset = load_dataset("bigcode/the-stack", "python", split="train[:10%]")  # 10% subset (~100k examples)

def tokenize_batch(batch):
    return tokenizer(batch["content"], truncation=True, max_length=seq_len, padding="max_length", return_tensors="pt")

# Process first batch for demo (use DataLoader in full run)
batch = tokenize_batch(dataset[:batch_size])
input_ids = batch["input_ids"].to(device)

# Simple embedding (proxy — use pre-trained embedder on better hardware)
embed = nn.Embedding(tokenizer.vocab_size, dim).to(device)
real_data = embed(input_ids)
...
class CodeCycleBlock(nn.Module):
    ...
    def forward(self, x, step):
        ...
        fused = (x_rot1 + Seq x_rot2 + x_rot3) / triality
        return fused
