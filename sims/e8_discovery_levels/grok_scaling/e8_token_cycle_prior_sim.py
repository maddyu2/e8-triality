...
# ────────────────────────────────────────────────
# CONFIG – optimized for speed & memory
# ────────────────────────────────────────────────
...
# Data
# ────────────────────────────────────────────────
mask_fraction = torch.linspace(0.4, 0.6, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # 50% masking proxy
token_coherence = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

token_sym = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

real_data = torch.cat([mask_fraction, token_coherence, token_sym], dim=-1)\
...
# Rotary – optimized
# ────────────────────────────────────────────────
class TokenCycleRotary(nn.Module):
    def __init__(self):
        super().__init__()
...
# ────────────────────────────────────────────────
class E8TokenCyclePrior(nn.Module):
    def __init__(self, depth=256):
        super().__init__()
        self.layers = nn.ModuleList([nn.MultiheadAttention(dim, triality, batch_first=True, dropout=0.0) for _ in range(depth)])
        self.rotary = TokenCycleRotary()
...
# ────────────────────────────────────────────────
model = E8TokenCyclePrior().to(device)
...
# ────────────────────────────────────────────────
model_ablation = E8TokenCyclePrior().to(device)
...
var_data = torch.cat([mask_fraction_var, token_coherence, token_sym], dim=-1)\
             .repeat(1, 1, dim // 3) * torch.randn(batch_size, seq_len, dim, device=device) * noise_scale
...
plt.tight_layout()
plt.savefig("token_cycle_prior_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as token_cycle_prior_ablation_precision_entropy.png")
