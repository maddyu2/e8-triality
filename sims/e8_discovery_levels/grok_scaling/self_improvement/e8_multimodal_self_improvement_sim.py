...
# ────────────────────────────────────────────────
# Multimodal self-improvement proxies (old/new task overlap 0.4–0.7, coherence gain 0.85–1.0)
old_task_overlap = torch.linspace(0.4, 0.7, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
new_task_emb = torch.linspace(0.5, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
self_coherence = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

self_sym = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

real_data = torch.cat([old_task_overlap, new_task_emb, self_coherence, self_sym], dim=-1)\
...
# ────────────────────────────────────────────────
# Triality Rotary Embeddings (cycle block)
# ────────────────────────────────────────────────
class TrialityRotaryEmbeddings(nn.Module):
    def __init__(self):
        super().__init__()
...
# ────────────────────────────────────────────────
# Model with rotary embeddings
# ────────────────────────────────────────────────
class E8MultimodalSelfImprovement(nn.Module):
    def __init__(self, depth=256, use_triality=True):
        ...
        self.rotary = TrialityRotaryEmbeddings()
...
# Training – optimized
# ────────────────────────────────────────────────
model = E8MultimodalSelfImprovement(use_triality=True).to(device)
...
# ────────────────────────────────────────────────
model_ablation = E8MultimodalSelfImprovement(use_triality=False).to(device)
...
plt.tight_layout()
plt.savefig("multimodal_self_improvement_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as multimodal_self_improvement_ablation_precision_entropy.png")
