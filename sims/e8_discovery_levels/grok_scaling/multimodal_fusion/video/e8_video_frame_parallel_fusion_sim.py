...
dim = 240
latent_dim = 8
seq_len = 1024  # represents frame sequence length
noise_scale = 0.002
batch_size = 128                     # increased — adjust to VRAM
micro_batch_size = 64
...
# ────────────────────────────────────────────────
# Sparse video frame proxies (30–70% missing frames, motion coherence 0.85–1.0)
missing_frames = torch.linspace(0.3, 0.7, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
frame_emb = torch.linspace(0.5, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # proxy
motion_coherence = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

video_sym = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

real_data = torch.cat([missing_frames, frame_emb, motion_coherence, video_sym], dim=-1)\
             .repeat(1, 1, dim // 4) * torch.randn(batch_size, seq_len, dim, device=device) * noise_scale

# Apply masking (simulate missing frames)
mask = torch.rand_like(real_data) < missing_frames
real_data[mask] = 0
...
class E8VideoFrameParallelFusion(nn.Module):
...
model = E8VideoFrameParallelFusion(use_triality=True).to(device)
model = torch.compile(model)  # Torch 2.0+ speedup
...
# Ablation
model_ablation = E8VideoFrameParallelFusion(use_triality=False).to(device)
...
plt.tight_layout()
plt.savefig("video_frame_parallel_fusion_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as video_frame_parallel_fusion_ablation_precision_entropy.png")
