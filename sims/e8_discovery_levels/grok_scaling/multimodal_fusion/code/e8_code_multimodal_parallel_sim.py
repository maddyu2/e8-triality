...
# ────────────────────────────────────────────────
# Sparse code + multimodal context proxies (40–70% missing code tokens, coherence 0.85–1.0)
...
video_image_emb = torch.linspace(0.5, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # proxy
...
real_data = torch.cat([missing_code, code_emb, video_image_emb, code_coherence, code_sym], dim=-1)\
             .repeat(1, 1, dim // 5) * torch.randn(batch_size, seq_len, dim, device=device) * noise_scale

# Apply masking to code part
mask = torch.rand_like(real_data) < missing_code
real_data[mask] = 0

# Coherence target (proxy for accurate code completion)
...
class E8CodeMultimodalParallel(nn.Module):
...
# Training – optimized
# ────────────────────────────────────────────────
model = E8CodeMultimodalParallel(use_triality=True).to(device)
...
# ────────────────────────────────────────────────
model_ablation = E8CodeMultimodalParallel(use_triality=False).to(device)
...
plt.tight_layout()
plt.savefig("code_multimodal_parallel_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as code_multimodal_parallel_ablation_precision_entropy.png")
