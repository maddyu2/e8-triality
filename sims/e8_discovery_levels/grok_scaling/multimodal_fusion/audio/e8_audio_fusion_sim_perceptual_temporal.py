...
temporal_weight = 0.05  # new temporal loss weight
...
def perceptual_loss(x, y):
    feat_x = vgg(x.view(-1, 3, 32, 32))
    feat_y = vgg(y.view(-1, 3, 32, 32))
    return torch.mean(torch.abs(feat_x - feat_y))

def temporal_loss(y):
    # L1 between consecutive time steps
    return torch.mean(torch.abs(y[:, :-1] - y[:, 1:]))

# ────────────────────────────────────────────────
# Synthetic audio waveform proxy
# ────────────────────────────────────────────────
t = torch.arange(seq_len, device=device) / sample_rate
freq = 440.0
audio_wave = torch.sin(2 * torch.pi * freq * t) + 0.1 * torch.randn_like(t)
audio_wave = audio_wave.unsqueeze(0).repeat(batch_size, 1)

real_data = audio_wave.unsqueeze(-1).repeat(1, 1, dim) * noise_scale

# Sparse masking (40–70%)
missing_audio = torch.linspace(0.4, 0.7, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
mask = torch.rand_like(real_data) < missing_audio
real_data[mask] = 0
...
class E8AudioFusion(nn.Module):
    def __init__(self, depth=256, use_triality=True):
        ...
        self.cycle_block = AudioCycleBlock()
...
# ────────────────────────────────────────────────
# Training – optimized with perceptual + temporal loss
# ────────────────────────────────────────────────
...
for epoch in range(epochs):
    ...
    for micro_step in range(grad_accum_steps):
        with torch.amp.autocast(device_type='cuda', dtype=torch.float16) if use_amp else nullcontext():
            ...
            # Temporal loss (L1 between consecutive time steps)
            temp_loss = temporal_loss(prec) * temporal_weight
            loss = mse_loss + perc_loss + temp_loss
    ...
    if epoch % 500 == 0:
        ...
        print(f"Epoch {epoch:5d} | Prec {p:.6f} | Ent {e:.6f} | Perc {perc:.4f} | Temp {temp:.4f} | LR {opt.param_groups[0]['lr']:.2e}")
...
for epoch in range(epochs):
    ...
    for micro_step in range(grad_accum_steps):
        with torch.amp.autocast(device_type='cuda', dtype=torch.float16) if use_amp else nullcontext():
            ...
            abl_temp = temporal_loss(abl_prec) * temporal_weight
            abl_loss = abl_mse + abl_perc + abl_temp
...
# ────────────────────────────────────────────────
mel_transform = T.MelScale(n_mels=n_mels, sample_rate=sample_rate, n_stft=n_fft // 2 + 1).to(device)

with torch.no_grad():
    reconstructed = model(real_data, 0)

original_wave = real_data[:, :, 0].cpu()
recon_wave = reconstructed[:, 0].cpu()

original_mels = []
recon_mels = []
for i in range(min(8, batch_size)):
    orig_wave = original_wave[i].to(device)
    recon_wave_t = recon_wave[i].to(device)

    orig_stft = torch.stft(orig_wave, n_fft=n_fft, hop_length=hop_length, return_complex=True)
    recon_stft = torch.stft(recon_wave_t, n_fft=n_fft, hop_length=hop_length, return_complex=True)

    orig_mel = mel_transform(orig_stft.abs()).cpu().numpy()
    recon_mel = mel_transform(recon_stft.abs()).cpu().numpy()

    original_mels.append(orig_mel)
    recon_mels.append(recon_mel)

fig, axes = plt.subplots(2, min(8, batch_size), figsize=(min(8, batch_size) * 2, 4), sharex=True, sharey=True)
for i in range(min(8, batch_size)):
    axes[0, i].imshow(original_mels[i], aspect='auto', origin='lower', cmap='viridis')
    axes[0, i].set_title(f"Masked #{i+1}")
    axes[0, i].axis('off')

    axes[1, i].imshow(recon_mels[i], aspect='auto', origin='lower', cmap='viridis')
    axes[1, i].set_title(f"Reconstructed #{i+1}")
    axes[1, i].axis('off')

plt.suptitle("Original Masked vs Triality Reconstructed Mel-Spectrograms")
plt.tight_layout()
plt.savefig("audio_fusion_mel_spectrogram_comparison.png", dpi=300, bbox_inches='tight')
plt.show()

print("Mel-spectrogram visualization saved as audio_fusion_mel_spectrogram_comparison.png")

# Perceptual & Temporal Loss Curves
plt.figure(figsize=(10, 4))
plt.plot(perc_hist, label='Perceptual Loss')
plt.plot(temp_hist, label='Temporal Loss')
plt.title("Perceptual and Temporal Loss Over Training")
plt.xlabel("Epoch / 500")
plt.ylabel("Loss")
plt.legend()
plt.grid(True)
plt.savefig("audio_fusion_perceptual_temporal_loss.png", dpi=300, bbox_inches='tight')
plt.show()

print("Loss curves saved as audio_fusion_perceptual_temporal_loss.png")
