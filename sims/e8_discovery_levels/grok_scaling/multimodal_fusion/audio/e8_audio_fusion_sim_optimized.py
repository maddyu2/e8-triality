...
# ────────────────────────────────────────────────
# Sparse audio proxies (40–70% masking, audio coherence 0.85–1.0)
missing_audio = torch.linspace(0.4, 0.7, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
audio_emb = torch.linspace(0.5, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # proxy
audio_coherence = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

audio_sym = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

real_data = torch.cat([missing_audio, audio_emb, audio_coherence, audio_sym], dim=-1)\
