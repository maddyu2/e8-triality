...
n_fft = 512                          # STFT parameters for spectrogram
hop_length = 256

# ────────────────────────────────────────────────
# Synthetic audio proxy (sine wave + noise + masking)
# ────────────────────────────────────────────────
t = torch.arange(seq_len, device=device) / sample_rate
freq = 440.0  # A4 note
audio_wave = torch.sin(2 * torch.pi * freq * t) + 0.1 * torch.randn_like(t)
audio_wave = audio_wave.unsqueeze(0).repeat(batch_size, 1)  # batch of identical waveforms

# Convert to complex STFT representation for fusion
audio_stft = torch.stft(audio_wave, n_fft=n_fft, hop_length=hop_length, return_complex=True)
audio_stft_real = audio_stft.real
audio_stft_imag = audio_stft.imag

# Flatten STFT for input to model (real + imag channels)
real_data = torch.cat([audio_stft_real, audio_stft_imag], dim=-1)  # shape: (batch, freq_bins, time_frames)

# Apply sparse masking (40–70% missing time frames)
missing_audio = torch.linspace(0.4, 0.7, batch_size * real_data.shape[1], device=device).view(batch_size, real_data.shape[1], 1)
mask = torch.rand_like(real_data) < missing_audio
real_data[mask] = 0

# Coherence target (proxy for clean reconstruction)
coherence_target = torch.ones_like(real_data.mean(dim=-1, keepdim=True))
...
# ────────────────────────────────────────────────
# Reconstruct from model output (proxy inverse STFT – simplified for demo)
with torch.no_grad():
    reconstructed = model(real_data, 0)  # final reconstruction proxy
    # For visualization, we use magnitude of original vs reconstructed embeddings
    original_spec = real_data.abs().mean(dim=0).cpu().numpy().squeeze()  # average over batch
    recon_spec = reconstructed.abs().mean(dim=0).cpu().numpy().squeeze()

fig, axs = plt.subplots(1, 2, figsize=(12, 5))
axs[0].imshow(original_spec, aspect='auto', origin='lower', cmap='viridis')
axs[0].set_title("Original Masked Spectrogram Proxy")
axs[0].set_xlabel("Time")
axs[0].set_ylabel("Frequency")

axs[1].imshow(recon_spec, aspect='auto', origin='lower', cmap='viridis')
axs[1].set_title("Triality Reconstructed Spectrogram Proxy")
...
print("Spectrogram comparison saved as audio_fusion_spectrogram_comparison.png")