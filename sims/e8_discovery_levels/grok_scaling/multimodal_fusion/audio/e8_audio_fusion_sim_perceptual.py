...
# ────────────────────────────────────────────────
# Pre-trained VGG16 for perceptual loss (audio proxy)
# ────────────────────────────────────────────────
...
def perceptual_loss(x, y):
    # Simple L1 on VGG features (proxy for audio perceptual similarity)
    feat_x = vgg(x.view(-1, 3, 32, 32))  # reshape to image-like for VGG
...
for epoch in range(epochs):
    ...
    if epoch % 500 == 0:
        ...
        abl_perc = abl_perc.item()
        abl_prec_hist.append(ap)
        abl_ent_hist.append(ae)
        abl_perceptual_hist.append(abl_perc)
...
plt.plot(perceptual_hist, label='Perceptual Loss')
plt.title("Perceptual Loss Over Training")
plt.xlabel("Epoch / 500")
plt.ylabel("Perceptual Loss")
plt.legend()
plt.grid(True)
plt.savefig("audio_fusion_perceptual_loss.png", dpi=300, bbox_inches='tight')
plt.show()

print("Perceptual loss curve saved as audio_fusion_perceptual_loss.png")
