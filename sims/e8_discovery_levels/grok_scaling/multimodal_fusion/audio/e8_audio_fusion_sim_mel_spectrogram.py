...
import torchaudio.transforms as T  # for MelScale
import numpy as np
...
n_fft = 256                          # more efficient STFT params
hop_length = 128
n_mels = 64                          # mel bins

# ────────────────────────────────────────────────
# Synthetic audio waveform proxy (sine wave + noise)
# ────────────────────────────────────────────────
t = torch.arange(seq_len, device=device) / sample_rate
freq = 440.0  # A4 note
audio_wave = torch.sin(2 * torch.pi * freq * t) + 0.1 * torch.randn_like(t)
audio_wave = audio_wave.unsqueeze(0).repeat(batch_size, 1)  # (batch, time)

# Convert to embedding space for fusion
real_data = audio_wave.unsqueeze(-1).repeat(1, 1, dim) * noise_scale  # (batch, seq, dim)

# Apply sparse masking (40–70% missing time steps)
...
# Coherence target (proxy for clean reconstruction)
...
with torch.no_grad():
    reconstructed = model(real_data, 0)  # final reconstruction proxy

# Reshape to waveform for STFT
original_wave = real_data[:, :, 0].cpu()  # take first channel as waveform proxy
recon_wave = reconstructed[:, 0].cpu()    # reconstructed scalar proxy (simplified)

original_mels = []
recon_mels = []
for i in range(min(8, batch_size)):  # visualize first 8
...
for i in range(min(8, batch_size)):
    ...
    axes[0, i].set_title(f"Masked Audio #{i+1}")
    axes[0, i].axis('off')
