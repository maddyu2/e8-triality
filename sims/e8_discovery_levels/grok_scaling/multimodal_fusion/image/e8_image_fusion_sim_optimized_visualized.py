...
# ────────────────────────────────────────────────
# Synthetic image proxies (simple gradient + noise, 40–70% masking)
# ────────────────────────────────────────────────
# Create batch of simple gradient images (proxy for real image patches)
x_grid, y_grid = torch.meshgrid(torch.linspace(-1, 1, 32, device=device), torch.linspace(-1, 1, 32, device=device), indexing='ij')
gradient = (x_grid + y_grid).unsqueeze(0).unsqueeze(0).repeat(batch_size, 3, 1, 1)  # 3 channels
image_data = gradient + 0.1 * torch.randn_like(gradient) * noise_scale
image_data = image_data.view(batch_size, 3 * 32 * 32).to(device)  # flatten to dim

# Apply sparse masking (40–70% missing pixels)
missing_image = torch.linspace(0.4, 0.7, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
mask = torch.rand_like(image_data) < missing_image.squeeze(-1)
image_data[mask] = 0
...
real_data = image_data.unsqueeze(1)  # add seq_len=1 for compatibility
...
class ImageCycleBlock(nn.Module):
    def __init__(self):
        super().__init__()
...
class E8ImageFusion(nn.Module):
    def __init__(self, depth=256, use_triality=True):
        ...
        self.cycle_block = ImageCycleBlock()
...
# Training – optimized
# ────────────────────────────────────────────────
model = E8ImageFusion(use_triality=True).to(device)
...
# ────────────────────────────────────────────────
model_ablation = E8ImageFusion(use_triality=False).to(device)
...
# Reshape for visualization (batch of flattened images → 32x32x3)
num_vis = min(8, batch_size)  # show first 8 examples
original_images = real_data[:num_vis].view(num_vis, 32, 32, 3).cpu().numpy()
recon_images = reconstructed[:num_vis].view(num_vis, 32, 32, 3).cpu().numpy()
...
plt.suptitle("Original Masked vs Triality Reconstructed Images")
plt.tight_layout()
plt.savefig("image_fusion_visualization.png", dpi=300, bbox_inches='tight')
plt.show()

print("Image visualization saved as image_fusion_visualization.png")
