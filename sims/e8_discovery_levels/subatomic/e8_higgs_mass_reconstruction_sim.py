...
noise_scale = 0.002
batch_size = 64
heads = triality  # Full triality

# Expanded mock data (2000 events)
np.random.seed(42)
n_events = 2000
m_gg = np.random.normal(125.0, 1.5, n_events)
pT_lead = np.random.uniform(25, 70, n_events)
delta_eta = np.random.uniform(0.4, 1.6, n_events)
delta_phi = np.random.uniform(0.4, 1.6, n_events)
data_np = np.column_stack((m_gg, pT_lead, delta_eta, delta_phi))
data = torch.tensor(data_np, dtype=torch.float32, device=device)

target_mass = torch.tensor([125.0] * n_events, device=device)

# Full E8 roots (8D → 240D projection)
def get_e8_roots():
    roots = []
    for i in range(8):
...
projection = nn.Linear(4, dim, bias=False).to(device)
...
class E8HiggsRecon(nn.Module):
    def __init__(self, depth=8):  # Start small for test, scale to 256
        super().__init__()
...
model = E8HiggsRecon(depth=8).to(device)  # Increase depth for full power
