...
# Synthetic Atlas proxy (proprioceptive states + vision features + noise/occlusion)
...
for b in range(batch_size):
    ...
    proprio = torch.sin(t.unsqueeze(-1) * torch.arange(features_proprio, device=device)) * 0.5  # gait cycles
    ...
    atlas_data.append(frame)

atlas_data = torch.stack(atlas_data).to(device)

# Project to dim
proj = nn.Linear(features_proprio + features_vision, dim).to(device)
clean_data = proj(atlas_data)
...
class AtlasCycleBlock(nn.Module):
    def __init__(self):
        super().__init__()
...
class E8AtlasFusion(nn.Module):
    def __init__(self, depth=32, use_triality=True):
        super().__init__()
        self.use_triality = use_triality
        self.cycle = AtlasCycleBlock() if use_triality else DummyCycle()
...
# Models
model = E8AtlasFusion(use_triality=True).to(device)
model_ablation = E8AtlasFusion(use_triality=False).to(device)
