...
dim = 384
latent_dim = 8
seq_len = 1024  # "measurement" steps
batch_size = 64  # larger (stable)
...
checkpoint_path = os.path.join(checkpoint_dir, "quantum_entanglement_checkpoint.pth")

# Synthetic quantum entanglement proxy (Bell pairs + decoherence noise/masking)
# Two "particles" with correlated features (entangled), add noise + masking (decoherence)
features_per_particle = 64

entangled_pairs = []
for b in range(batch_size):
    t = torch.linspace(0, 10*math.pi, seq_len, device=device)
    # Correlated base (entangled)
    base = torch.sin(t.unsqueeze(-1) * torch.arange(features_per_particle, device=device)) * 0.5
    particle1 = base + torch.randn_like(base) * 0.05  # slight independent noise
    particle2 = base.flip(-1) + torch.randn_like(base) * 0.05  # correlated flip (Bell-like)
    
    pair = torch.cat([particle1, particle2], dim=-1)  # (seq_len, 2*features)
    pair += torch.randn_like(pair) * 0.1  # decoherence noise
    entangled_pairs.append(pair)

entangled_pairs = torch.stack(entangled_pairs).to(device)

# Project to dim
proj = nn.Linear(features_per_particle * 2, dim).to(device)
clean_data = proj(entangled_pairs)
...
class QuantumCycleBlock(nn.Module):
    def __init__(self):
        super().__init__()
...
class E8QuantumFusion(nn.Module):
    def __init__(self, depth=32, use_triality=True):
        super().__init__()
        self.use_triality = use_triality
        self.cycle = QuantumCycleBlock() if use_triality else DummyCycle()
...
# Models
model = E8QuantumFusion(use_triality=True).to(device)
model_ablation = E8QuantumFusion(use_triality=False).to(device)
