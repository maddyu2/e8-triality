...
dim = 384
latent_dim = 8
seq_len = 1024  # time steps (landing burn sequence)
batch_size = 64
...
checkpoint_path = os.path.join(checkpoint_dir, "starship_landing_checkpoint.pth")

# Synthetic Starship landing thruster proxy (Raptor thrust vectors + turbulence noise/occlusion)
features_thrust = 128  # vector components

thruster_data = []
for b in range(batch_size):
    t = torch.linspace(0, 10*math.pi, seq_len, device=device)
    
    # Coherent landing trajectory (belly-flop + burn patterns)
    thrust = torch.sin(t.unsqueeze(-1) * torch.arange(features_thrust, device=device)) * 0.5
    
    # Turbulence noise (atmospheric chaos proxy)
    thrust += torch.randn_like(thrust) * 0.2
    
    thruster_data.append(thrust)

thruster_data = torch.stack(thruster_data).to(device)

# Project to shared dim
proj = nn.Linear(features_thrust, dim).to(device)
clean_data = proj(thruster_data)

# High masking (70–90% — sensor dropout/turbulence proxy)
...
class LandingCycleBlock(nn.Module):
    def __init__(self):
        super().__init__()
...
class E8LandingFusion(nn.Module):
    def __init__(self, depth=32, use_triality=True):
        super().__init__()
        self.use_triality = use_triality
        self.cycle = LandingCycleBlock() if use_triality else DummyCycle()
...
# Models
model = E8LandingFusion(use_triality=True).to(device)
model_ablation = E8LandingFusion(use_triality=False).to(device)
...
# Visualization: Thrust Trajectory Reconstruction (first feature channel proxy)
model.eval()
model_ablation.eval()
...
with torch.no_grad():
    ...
    # Plot first thrust feature channel (trajectory proxy)
    ...
    for i in range(8):
        ...
        axes[0, i].set_title("Original Thrust")
    ...
    plt.suptitle("E8 Triality Starship Landing Thruster Control Visualization")
    plt.tight_layout()
    plt.show()
