...
dim = 384
latent_dim = 8
seq_len = 1024  # time steps (entry trajectory)
batch_size = 64
...
checkpoint_path = os.path.join(checkpoint_dir, "starship_entry_checkpoint.pth")

# Synthetic Starship atmosphere entry proxy (heat shield + sensor noise/occlusion)
features_entry = 128  # trajectory/heat features

entry_data = []
for b in range(batch_size):
    t = torch.linspace(0, 10*math.pi, seq_len, device=device)
    
    # Coherent entry trajectory (plasma blackout patterns)
    entry = torch.sin(t.unsqueeze(-1) * torch.arange(features_entry, device=device)) * 0.5
    
    # Plasma/turbulence noise
    entry += torch.randn_like(entry) * 0.25
    
    entry_data.append(entry)

entry_data = torch.stack(entry_data).to(device)

# Project to shared dim
proj = nn.Linear(features_entry, dim).to(device)
clean_data = proj(entry_data)

# High masking (70–90% — plasma blackout/sensor dropout proxy)
...
class EntryCycleBlock(nn.Module):
    def __init__(self):
        super().__init__()
...
class E8EntryFusion(nn.Module):
    def __init__(self, depth=32, use_triality=True):
        super().__init__()
        self.use_triality = use_triality
        self.cycle = EntryCycleBlock() if use_triality else DummyCycle()
...
# Models
model = E8EntryFusion(use_triality=True).to(device)
model_ablation = E8EntryFusion(use_triality=False).to(device)
...
# Visualization: Entry Trajectory Reconstruction (first feature channel proxy)
model.eval()
model_ablation.eval()
...
with torch.no_grad():
    ...
    # Plot first entry feature channel (trajectory proxy)
    ...
    for i in range(8):
        ...
        axes[0, i].set_title("Original Entry")
    ...
    plt.suptitle("E8 Triality Starship Atmosphere Entry Visualization")
    plt.tight_layout()
    plt.show()
