...
dim = 384
latent_dim = 8
seq_len = 1024  # time steps (spike train length)
batch_size = 64
...
checkpoint_path = os.path.join(checkpoint_dir, "neuralink_spike_checkpoint.pth")

# Synthetic Neuralink spike decoding proxy (multi-channel spikes + drift/noise/occlusion)
channels = 128  # multi-electrode proxy

spike_data = []
for b in range(batch_size):
    t = torch.linspace(0, 10*math.pi, seq_len, device=device)
    # Correlated spike patterns (thought-like bursts)
    base = torch.sin(t.unsqueeze(-1) * torch.arange(channels, device=device)) * 0.5
    # Electrode drift over time
    drift = torch.linspace(0, 0.4, seq_len, device=device).unsqueeze(-1) * torch.randn(channels, device=device)
    spikes = base + drift
    spikes += torch.randn_like(spikes) * 0.15  # noise
    spike_data.append(spikes)

spike_data = torch.stack(spike_data).to(device)

# Project to dim
proj = nn.Linear(channels, dim).to(device)
clean_data = proj(spike_data)

# High masking (70–90% — electrode dropout/decoding gaps proxy)
...
class NeuralinkCycleBlock(nn.Module):
    def __init__(self):
        super().__init__()
...
class E8NeuralinkFusion(nn.Module):
    def __init__(self, depth=32, use_triality=True):
        super().__init__()
        self.use_triality = use_triality
        self.cycle = NeuralinkCycleBlock() if use_triality else DummyCycle()
...
# Models
model = E8NeuralinkFusion(use_triality=True).to(device)
model_ablation = E8NeuralinkFusion(use_triality=False).to(device)
...
# Visualization: Spike Train Reconstruction (first channel proxy)
model.eval()
model_ablation.eval()
...
with torch.no_grad():
    ...
    for b in range(8):
        ...
        drift = torch.linspace(0, 0.4, seq_len, device=device).unsqueeze(-1) * torch.randn(channels, device=device)
        spikes = base + drift
        spikes += torch.randn_like(spikes) * 0.15
        test_data.append(spikes)
    ...
    # Plot first channel (spike proxy)
    ...
    for i in range(8):
        ...
        axes[0, i].set_title("Original Spike")
    ...
    plt.suptitle("E8 Triality Neuralink Spike Decoding Visualization")
    plt.tight_layout()
    plt.show()
