...
checkpoint_path = os.path.join(checkpoint_dir, "neuralink_spike_drift_checkpoint.pth")

# Synthetic Neuralink spike drift correction proxy (correlated spike trains + drift/noise/masking)
features_per_channel = 64  # multi-channel spikes

spike_data = []
for b in range(batch_size):
    t = torch.linspace(0, 10*math.pi, seq_len, device=device)
    # Correlated base spikes (neural patterns)
    base = torch.sin(t.unsqueeze(-1) * torch.arange(features_per_channel, device=device)) * 0.5
    # Drift over time (electrode shift proxy)
    drift = torch.linspace(0, 0.3, seq_len, device=device).unsqueeze(-1) * torch.randn(features_per_channel, device=device)
    spikes = base + drift
    spikes += torch.randn_like(spikes) * 0.1  # noise
    spike_data.append(spikes)

spike_data = torch.stack(spike_data).to(device)

# Project to dim
proj = nn.Linear(features_per_channel, dim).to(device)
...
class SpikeCycleBlock(nn.Module):
    def __init__(self):
        super().__init__()
...
class E8SpikeFusion(nn.Module):
    def __init__(self, depth=32, use_triality=True):
        super().__init__()
        self.use_triality = use_triality
        self.cycle = SpikeCycleBlock() if use_triality else DummyCycle()
...
# Models
model = E8SpikeFusion(use_triality=True).to(device)
model_ablation = E8SpikeFusion(use_triality=False).to(device)
