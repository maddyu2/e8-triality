...
checkpoint_path = os.path.join(checkpoint_dir, "neuralink_brain_signal_checkpoint.pth")

# Synthetic Neuralink brain signal stability proxy (multi-channel spikes + drift/noise/occlusion)
channels = 128  # multi-electrode proxy

brain_signals = []
for b in range(batch_size):
    t = torch.linspace(0, 10*math.pi, seq_len, device=device)
    # Correlated spike patterns (thought-like)
    ...
    signals = base + drift
    signals += torch.randn_like(signals) * 0.15  # noise
    brain_signals.append(signals)

brain_signals = torch.stack(brain_signals).to(device)

# Project to dim
proj = nn.Linear(channels, dim).to(device)
clean_data = proj(brain_signals)
...
class BrainCycleBlock(nn.Module):
    def __init__(self):
        super().__init__()
...
class E8BrainFusion(nn.Module):
    def __init__(self, depth=32, use_triality=True):
        super().__init__()
        self.use_triality = use_triality
        self.cycle = BrainCycleBlock() if use_triality else DummyCycle()
...
# Models
model = E8BrainFusion(use_triality=True).to(device)
model_ablation = E8BrainFusion(use_triality=False).to(device)
