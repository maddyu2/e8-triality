...
# CONFIG – optimized for no OOM + speed (fits A100/H100)
triality = 3
dim = 192  # reduced
latent_dim = 8
seq_len = 1024  # reduced (long-horizon proxy)
...
# Long-horizon sparse text proxy
...
target = long_seq.repeat(1, 1, dim)
...
class LongHorizonCycleBlock(nn.Module):
    ...
    def forward(self, x, step):
        ...
        pump_broadcast = pump.unsqueeze(0).unsqueeze(0)
        x_rot1 = x * (emb_broadcast.cos() + pump_broadcast)
...
class E8LongHorizon(nn.Module):
    def __init__(self, depth=16):  # reduced
        super().__init__()
        self.cycle = LongHorizonCycleBlock()
...
# Visualization (long-horizon precision/entropy curves — simple)
with torch.no_grad():
...
plt.suptitle("Long Horizon Token Eternal Lock: Masked vs Triality Reconstructed")
plt.tight_layout()
plt.show()
