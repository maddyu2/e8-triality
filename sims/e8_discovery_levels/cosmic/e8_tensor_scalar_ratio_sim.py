...
# Planck/BICEP 2025 proxies (r <0.036, B-mode C_ℓ^BB ~0.01-0.1 μK², tensor spectral index n_t ~ -r/8)
# Vary tensor_ratio / b_mode_power as input vectors
...
# Tensor symmetry proxy (~0.85–1.0)
tensor_sym = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

real_data = torch.cat([tensor_ratio, b_mode_power, tensor_sym], dim=-1)\
...
# Sectors: Tensor ratio, B-mode power, Tensor symmetry, Prediction nulling
ratio_roots = e8_roots[:80]
bmode_roots = e8_roots[80:160]
sym_roots = e8_roots[160:]

class TensorRatioRotary(nn.Module):
    def __init__(self):
        super().__init__()
...
# Initial tensor-to-scalar state → precision target
states = real_data
target_prec = torch.ones(batch_size, 1, device=device)

model = E8TensorScalarRatio().to(device)
...
plt.tight_layout()
plt.savefig("tensor_scalar_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as tensor_scalar_precision_entropy.png")
