...
# LISA 2025 proxies (strain ~10^{-23}, freq 0.1 mHz–1 Hz, SMBH merger masses 10^5–10^7 M_⊙)
# Vary gw_strain / frequency / bh_mass as input vectors
gw_strain = torch.linspace(1e-24, 1e-22, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
frequency = torch.linspace(1e-4, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # Hz
bh_mass = torch.linspace(1e5, 1e7, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # M_⊙

# Spacetime symmetry proxy (~0.85–1.0)
spacetime_sym = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

real_lisa_data = torch.cat([gw_strain, frequency, bh_mass, spacetime_sym], dim=-1).repeat(1, 1, dim // 4) * torch.randn(batch_size, seq_len, dim, device=device) * 0.01
...
# Sectors: GW strain, Frequency, BH mass, Spacetime symmetry, Prediction nulling
strain_roots = e8_roots[:60]
freq_roots = e8_roots[60:120]
mass_roots = e8_roots[120:180]
sym_roots = e8_roots[180:]

class LISAGWRotary(nn.Module):
    def __init__(self):
        super().__init__()
...
class E8LISAGW(nn.Module):
    def __init__(self, depth=256):
        super().__init__()
        subsets = [strain_roots, freq_roots, mass_roots, sym_roots]
...
# Initial LISA GW state → precision target
states = real_lisa_data
target_prec = torch.ones(batch_size, 1, device=device)

model = E8LISAGW().to(device)
...
plt.tight_layout()
plt.savefig("lisa_gw_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as lisa_gw_precision_entropy.png")
