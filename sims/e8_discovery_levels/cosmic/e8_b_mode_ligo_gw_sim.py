...
# Planck/BICEP 2025 + LIGO O4 proxies (B-mode C_ℓ^BB ~0.01-0.1 μK² for r~0.01, GW strain ~10^{-21})
# Vary b_mode_power / tensor_ratio / gw_strain as input vectors
b_mode_power = torch.linspace(0.01, 0.1, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # μK² proxy
tensor_ratio = torch.linspace(0.001, 0.036, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
gw_strain = torch.linspace(1e-22, 1e-20, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

# Polarization symmetry proxy (~0.85–1.0)
polar_sym = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

real_data = torch.cat([b_mode_power, tensor_ratio, gw_strain, polar_sym], dim=-1)\
...
plt.tight_layout()
plt.savefig("cmb_gw_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as cmb_gw_precision_entropy.png")
