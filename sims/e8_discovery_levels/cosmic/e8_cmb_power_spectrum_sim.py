...
# Planck 2018 legacy + 2025 ACT/SPT proxies
# TT power spectrum C_ℓ (μK²) for ℓ ~2–2500, peak at ℓ~200 ~6000 μK²
cmb_power = torch.linspace(1000, 6000, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # peak multipole range
multipole = torch.linspace(2, 2500, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)     # ℓ
cmb_sym = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)     # isotropy symmetry

real_cmb_data = torch.cat([cmb_power, multipole, cmb_sym], dim=-1)\
...
class E8CMBPowerSpectrum(nn.Module):
...
# Training
model = E8CMBPowerSpectrum().to(device)
...
for epoch in range(epochs):
    opt.zero_grad()
    prec = model(real_cmb_data, epoch)
...
plt.tight_layout()
plt.savefig("cmb_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as cmb_precision_entropy.png")
