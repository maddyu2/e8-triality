...
# Second cell: The sim code (optimized — 5000 epochs, checkpoints, visualization)
import torch
...
checkpoint_path = os.path.join(checkpoint_dir, "image_recon_checkpoint.pth")
...
test_loader = DataLoader(mnist_test, batch_size=16, shuffle=False)  # small for viz
...
class E8ImageRecon(nn.Module):
    def __init__(self, depth=16, use_triality=True):  # reduced depth
        ...
        self.cycle = ImageCycleBlock() if use_triality else DummyCycle()
...
# Models
model = E8ImageRecon(use_triality=True).to(device)
model_ablation = E8ImageRecon(use_triality=False).to(device)
