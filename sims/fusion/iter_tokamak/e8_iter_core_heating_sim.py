...
# ITER 2025 core heating proxies (NBI power ~20–50 MW, ICRH ~20–40 MW, alpha fraction ~0.3–0.6)
nbi_power = torch.linspace(20, 50, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # MW
icrh_power = torch.linspace(20, 40, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
alpha_frac = torch.linspace(0.3, 0.6, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

heating_sym = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

real_data = torch.cat([nbi_power, icrh_power, alpha_frac, heating_sym], dim=-1)\
...
# Sectors
nbi_roots = e8_roots[:60]
icrh_roots = e8_roots[60:120]
alpha_roots = e8_roots[120:180]
sym_roots = e8_roots[180:]

class CoreHeatingRotary(nn.Module):
    def __init__(self):
        super().__init__()
...
model = E8ITERCoreHeating().to(device)
...
class AblationModel(E8ITERCoreHeating):
    def __init__(self, depth=256):
...
plt.tight_layout()
plt.savefig("iter_core_heating_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as iter_core_heating_ablation_precision_entropy.png")
