...
# ITER ELM mitigation proxies (RMP coil current ~1–5 kA, pedestal beta ~1.0–3.0, ELM energy loss <0.5 MJ/m²)
...
elm_energy = torch.linspace(0.1, 0.5, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # MJ/m²

elm_sym = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

real_data = torch.cat([rmp_current, ped_beta, elm_energy, elm_sym], dim=-1)\
...
# Sectors: RMP current, Ped beta, ELM energy, ELM symmetry, Prediction nulling
...
class E8ITERELMMitigation(nn.Module):
...
# Initial ELM mitigation state → precision target
states = real_data
target_prec = torch.ones(batch_size, 1, device=device)

model = E8ITERELMMitigation().to(device)
...
var_data = torch.cat([rmp_current_var, ped_beta, elm_energy, elm_sym], dim=-1)\
             .repeat(1, 1, dim // 4) * torch.randn(batch_size, seq_len, dim, device=device) * noise_scale
...
class AblationModel(E8ITERELMMitigation):
    def __init__(self, depth=256):
...
plt.tight_layout()
plt.savefig("iter_elm_mitigation_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as iter_elm_mitigation_ablation_precision_entropy.png")
