...
# ITER MHD stability proxies (β_N ~2.0–3.5, q-profile 1.5–3.0, mode growth rate γ ~0.01–0.1 /τ_A)
beta_n = torch.linspace(2.0, 3.5, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
q_profile = torch.linspace(1.5, 3.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
...
real_data = torch.cat([beta_n, q_profile, mode_growth, mhd_sym], dim=-1)\
...
# Sectors: β_N, q-profile, Mode growth, MHD symmetry, Prediction nulling
beta_roots = e8_roots[:60]
q_roots = e8_roots[60:120]
growth_roots = e8_roots[120:180]
...
class E8ITERMHDStability(nn.Module):
...
model = E8ITERMHDStability().to(device)
...
class AblationModel(E8ITERMHDStability):
    def __init__(self, depth=256):
...
plt.tight_layout()
plt.savefig("iter_mhd_stability_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as iter_mhd_stability_ablation_precision_entropy.png")
