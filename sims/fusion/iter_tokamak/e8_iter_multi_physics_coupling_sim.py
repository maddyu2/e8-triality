...
# ITER multi-physics coupling proxies (SOL flux 5–15 MW/m², pedestal gradient 1–10, impurity density 1e-6–1e-5, radiation fraction 0.6–0.8)
sol_flux = torch.linspace(5, 15, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # MW/m²
ped_gradient = torch.linspace(1, 10, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
impurity_density = torch.linspace(1e-6, 1e-5, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
rad_fraction = torch.linspace(0.6, 0.8, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

multi_sym = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

real_data = torch.cat([sol_flux, ped_gradient, impurity_density, rad_fraction, multi_sym], dim=-1)\
...
# Sectors: SOL flux, Ped gradient, Impurity density, Radiation fraction, Multi-physics symmetry, Prediction nulling
sol_roots = e8_roots[:48]
ped_roots = e8_roots[48:96]
imp_roots = e8_roots[96:144]
rad_roots = e8_roots[144:192]
sym_roots = e8_roots[192:]

class MultiPhysicsRotary(nn.Module):
    def __init__(self):
        super().__init__()
...
# Initial multi-physics state → precision target
states = real_data
target_prec = torch.ones(batch_size, 1, device=device)

model = E8ITERMultiPhysicsCoupling().to(device)
...
# ────────────────────────────────────────────────
# Sensitivity Analysis: Vary impurity_density ±10%
# ────────────────────────────────────────────────

impurity_density_var = impurity_density * torch.linspace(0.9, 1.1, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
var_data = torch.cat([sol_flux, ped_gradient, impurity_density_var, rad_fraction, multi_sym], dim=-1)\
             .repeat(1, 1, dim // 5) * torch.randn(batch_size, seq_len, dim, device=device) * noise_scale

var_prec = model(var_data, epochs - 1)
var_ent = -var_prec * torch.log(var_prec + 1e-12)
var_variance = torch.var(var_prec.mean(dim=0)).item()
print(f"Sensitivity variance on precision (impurity density ±10%): {var_variance:.6f} — low variance = robust")

# ────────────────────────────────────────────────
# Sigma Test: Ablation baseline (no triality)
# ────────────────────────────────────────────────

class AblationModel(E8ITERMultiPhysicsCoupling):
    def __init__(self, depth=256):
        super().__init__(depth=256)
...
plt.tight_layout()
plt.savefig("iter_multi_physics_coupling_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as iter_multi_physics_coupling_ablation_precision_entropy.png")
