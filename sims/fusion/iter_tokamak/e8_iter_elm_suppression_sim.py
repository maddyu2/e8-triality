...
# ITER ELM suppression proxies (RMP current 1–5 kA, pedestal gradient 1–10, ELM energy loss <0.5 MJ/m²)
...
elm_loss = torch.linspace(0.1, 0.5, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # MJ/m²

elm_sym = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

real_data = torch.cat([rmp_current, ped_gradient, elm_loss, elm_sym], dim=-1)\
...
# Sectors: RMP current, Ped gradient, ELM loss, ELM symmetry, Prediction nulling
...
class E8ITERELMSuppression(nn.Module):
...
# Initial ELM suppression state → precision target
states = real_data
target_prec = torch.ones(batch_size, 1, device=device)

model = E8ITERELMSuppression().to(device)
...
var_data = torch.cat([rmp_current_var, ped_gradient, elm_loss, elm_sym], dim=-1)\
             .repeat(1, 1, dim // 4) * torch.randn(batch_size, seq_len, dim, device=device) * noise_scale
...
class AblationModel(E8ITERELMSuppression):
    def __init__(self, depth=256):
...
plt.tight_layout()
plt.savefig("iter_elm_suppression_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as iter_elm_suppression_ablation_precision_entropy.png")
