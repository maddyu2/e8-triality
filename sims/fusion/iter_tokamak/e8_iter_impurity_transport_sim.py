...
# ITER impurity transport proxies (W concentration <10^{-5}, edge flow ~100–500 m/s, impurity flux <1 m^{-2}s^{-1})
w_concentration = torch.linspace(1e-6, 1e-5, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
edge_flow = torch.linspace(100, 500, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # m/s
impurity_flux = torch.linspace(0.1, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # m^{-2}s^{-1}

impurity_sym = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

real_data = torch.cat([w_concentration, edge_flow, impurity_flux, impurity_sym], dim=-1)\
             .repeat(1, 1, dim // 4) * torch.randn(batch_size, seq_len, dim, device=device) * noise_scale

# E8 roots (same as above)

# Sectors: W concentration, Edge flow, Impurity flux, Impurity symmetry, Prediction nulling
w_roots = e8_roots[:60]
flow_roots = e8_roots[60:120]
flux_roots = e8_roots[120:180]
sym_roots = e8_roots[180:]

class ImpurityRotary(nn.Module):
    def __init__(self):
        super().__init__()
...
# Initial impurity transport state → precision target
states = real_data
target_prec = torch.ones(batch_size, 1, device=device)

model = E8ITERImpurityTransport().to(device)
...
# ────────────────────────────────────────────────
# Sensitivity Analysis: Vary w_concentration ±10%
# ────────────────────────────────────────────────

w_concentration_var = w_concentration * torch.linspace(0.9, 1.1, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
var_data = torch.cat([w_concentration_var, edge_flow, impurity_flux, impurity_sym], dim=-1)\
             .repeat(1, 1, dim // 4) * torch.randn(batch_size, seq_len, dim, device=device) * noise_scale

var_prec = model(var_data, epochs - 1)
var_ent = -var_prec * torch.log(var_prec + 1e-12)
var_variance = torch.var(var_prec.mean(dim=0)).item()
print(f"Sensitivity variance on precision (w_concentration ±10%): {var_variance:.6f} — low variance = robust")

# ────────────────────────────────────────────────
# Sigma Test: Ablation baseline (no triality)
# ────────────────────────────────────────────────

class AblationModel(E8ITERImpurityTransport):
    def __init__(self, depth=256):
...
plt.tight_layout()
plt.savefig("iter_impurity_transport_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as iter_impurity_transport_ablation_precision_entropy.png")
