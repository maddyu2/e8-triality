...
triality = 3
heads = triality  # Fix for undefined heads variable
dim = 240
latent_dim = 8
seq_len = 1024
noise_scale = 0.002
...
# JT-60SA ELM stability proxies (pedestal pressure ~50–150 kPa, gradient 1–10, ELM energy loss <0.5 MJ/m², β_N ~1.5–2.5)
ped_pressure = torch.linspace(50, 150, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # kPa proxy
...
beta_n = torch.linspace(1.5, 2.5, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

elm_sym = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

real_data = torch.cat([ped_pressure, ped_gradient, elm_energy_loss, beta_n, elm_sym], dim=-1)\
...
# Sectors: Ped pressure, Ped gradient, ELM energy loss, β_N, ELM symmetry, Prediction nulling
pressure_roots = e8_roots[:48]
gradient_roots = e8_roots[48:96]
energy_roots = e8_roots[96:144]
beta_roots = e8_roots[144:192]
sym_roots = e8_roots[192:]

class JT60SAELMRotary(nn.Module):
    def __init__(self):
        super().__init__()
...
# Initial JT-60SA ELM state → precision target
states = real_data
target_prec = torch.ones(batch_size, 1, device=device)

model = E8JT60SAELMStability().to(device)
...
var_data = torch.cat([ped_pressure_var, ped_gradient, elm_energy_loss, beta_n, elm_sym], dim=-1)\
             .repeat(1, 1, dim // 5) * torch.randn(batch_size, seq_len, dim, device=device) * noise_scale
...
class AblationModel(E8JT60SAELMStability):
    def __init__(self, depth=256):
...
plt.tight_layout()
plt.savefig("jt60sa_elm_stability_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as jt60sa_elm_stability_ablation_precision_entropy.png")
