...
# Sectors: B_t, I_p, β_N, Core symmetry, Prediction nulling
bt_roots = e8_roots[:60]
ip_roots = e8_roots[60:120]
beta_roots = e8_roots[120:180]
sym_roots = e8_roots[180:]

class ITERCoreRotary(nn.Module):
    def __init__(self):
        super().__init__()
...
# Training
model = E8ITERCorePlasma().to(device)
...
# ────────────────────────────────────────────────
# Sensitivity Analysis: Vary inputs ±10%
# ────────────────────────────────────────────────

# Vary B_t ±10%
b_t_var = b_t * torch.linspace(0.9, 1.1, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
var_data = torch.cat([b_t_var, i_p, beta_n, core_sym], dim=-1)\
             .repeat(1, 1, dim // 4) * torch.randn(batch_size, seq_len, dim, device=device) * noise_scale

var_prec = model(var_data, epochs - 1)  # use trained model
...
class AblationModel(E8ITERCorePlasma):
    def __init__(self, depth=256):
...
plt.tight_layout()
plt.savefig("iter_core_plasma_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as iter_core_plasma_ablation_precision_entropy.png")