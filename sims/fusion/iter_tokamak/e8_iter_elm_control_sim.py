...
# ITER ELM control proxies (RMP coil current ~1–5 kA, pedestal beta ~1.0–3.0, ELM energy loss <0.5 MJ/m²)
...
model = E8ITERELMControl().to(device)
...
var_data = torch.cat([rmp_current_var, ped_beta, elm_energy_loss, elm_sym], dim=-1)\
             .repeat(1, 1, dim // 4) * torch.randn(batch_size, seq_len, dim, device=device) * noise_scale
...
class AblationModel(E8ITERELMControl):
    def __init__(self, depth=256):
