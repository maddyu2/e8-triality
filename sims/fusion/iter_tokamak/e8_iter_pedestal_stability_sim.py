...
# ITER pedestal stability proxies (pressure ~100–200 kPa, gradient 1–10, ELM threshold 0.5–1.0)
ped_pressure = torch.linspace(100, 200, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # kPa proxy
ped_gradient = torch.linspace(1, 10, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
elm_threshold = torch.linspace(0.5, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

pedestal_sym = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

real_data = torch.cat([ped_pressure, ped_gradient, elm_threshold, pedestal_sym], dim=-1)\
...
# Sectors: Ped pressure, Ped gradient, ELM threshold, Pedestal symmetry, Prediction nulling
pressure_roots = e8_roots[:60]
gradient_roots = e8_roots[60:120]
elm_roots = e8_roots[120:180]
sym_roots = e8_roots[180:]

class PedestalStabilityRotary(nn.Module):
    def __init__(self):
        super().__init__()
...
# Initial pedestal stability state → precision target
states = real_data
target_prec = torch.ones(batch_size, 1, device=device)

model = E8ITERPedestalStability().to(device)
...
# ────────────────────────────────────────────────
# Sensitivity Analysis: Vary ped_pressure ±10%
# ────────────────────────────────────────────────

ped_pressure_var = ped_pressure * torch.linspace(0.9, 1.1, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
var_data = torch.cat([ped_pressure_var, ped_gradient, elm_threshold, pedestal_sym], dim=-1)\
             .repeat(1, 1, dim // 4) * torch.randn(batch_size, seq_len, dim, device=device) * noise_scale

var_prec = model(var_data, epochs - 1)
var_ent = -var_prec * torch.log(var_prec + 1e-12)
var_variance = torch.var(var_prec.mean(dim=0)).item()
print(f"Sensitivity variance on precision (ped_pressure ±10%): {var_variance:.6f} — low variance = robust")

# ────────────────────────────────────────────────
# Sigma Test: Ablation baseline (no triality)
# ────────────────────────────────────────────────

class AblationModel(E8ITERPedestalStability):
    def __init__(self, depth=256):
...
plt.tight_layout()
plt.savefig("iter_pedestal_stability_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as iter_pedestal_stability_ablation_precision_entropy.png")
