...
# ITER plasma profiles proxies (core n_e ~10^{20} m^{-3}, T_i ~10–20 keV, profile stability factor 0.85–1.0)
n_e_core = torch.linspace(5e19, 1.5e20, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # m^{-3}
t_i_core = torch.linspace(10, 20, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # keV
profile_stability = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

profile_sym = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

real_data = torch.cat([n_e_core, t_i_core, profile_stability, profile_sym], dim=-1)\
...
# Sectors: n_e core, T_i core, Profile stability, Profile symmetry, Prediction nulling
ne_roots = e8_roots[:60]
ti_roots = e8_roots[60:120]
stab_roots = e8_roots[120:180]
sym_roots = e8_roots[180:]

class PlasmaProfileRotary(nn.Module):
    def __init__(self):
        super().__init__()
...
# Initial plasma profile state → precision target
states = real_data
target_prec = torch.ones(batch_size, 1, device=device)

model = E8ITERPlasmaProfilesSym().to(device)
...
# ────────────────────────────────────────────────
# Sensitivity Analysis: Vary n_e_core ±10%
# ────────────────────────────────────────────────

n_e_core_var = n_e_core * torch.linspace(0.9, 1.1, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
var_data = torch.cat([n_e_core_var, t_i_core, profile_stability, profile_sym], dim=-1)\
             .repeat(1, 1, dim // 4) * torch.randn(batch_size, seq_len, dim, device=device) * noise_scale

var_prec = model(var_data, epochs - 1)
var_ent = -var_prec * torch.log(var_prec + 1e-12)
var_variance = torch.var(var_prec.mean(dim=0)).item()
print(f"Sensitivity variance on precision (n_e_core ±10%): {var_variance:.6f} — low variance = robust")

# ────────────────────────────────────────────────
# Sigma Test: Ablation baseline (no triality)
# ────────────────────────────────────────────────

class AblationModel(E8ITERPlasmaProfilesSym):
    def __init__(self, depth=256):
...
plt.tight_layout()
plt.savefig("iter_plasma_profiles_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as iter_plasma_profiles_ablation_precision_entropy.png")
