...
# ITER alpha self-heating proxies (alpha power 50–150 MW, DT ratio 0.4–0.6, confinement 3–5 s)
alpha_power = torch.linspace(50, 150, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # MW
dt_ratio = torch.linspace(0.4, 0.6, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
conf_time = torch.linspace(3, 5, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

alpha_sym = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

real_data = torch.cat([alpha_power, dt_ratio, conf_time, alpha_sym], dim=-1)\
...
# Sectors: Alpha power, DT ratio, Confinement time, Alpha symmetry, Prediction nulling
power_roots = e8_roots[:60]
ratio_roots = e8_roots[60:120]
time_roots = e8_roots[120:180]
sym_roots = e8_roots[180:]

class AlphaHeatingRotary(nn.Module):
    def __init__(self):
        super().__init__()
...
# Training
model = E8ITERAlphaSelfHeating().to(device)
...
class AblationModel(E8ITERAlphaSelfHeating):
    def __init__(self, depth=256):
...
sigma_ent = (abl_ent_mean - e8_ent_mean) / ent_std if ent_std > 0 else 0  # lower entropy is better
...
plt.tight_layout()
plt.savefig("iter_alpha_self_heating_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as iter_alpha_self_heating_ablation_precision_entropy.png")
