...
# SPARC reactor design proxies (high-field compact tokamak: B_t ~12 T, I_p ~8–10 MA, β_N ~2.0–4.0, Q >10, H98 ~1.3–1.6)
b_t = torch.linspace(11.5, 12.5, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # T
i_p = torch.linspace(8.0, 10.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # MA
beta_n = torch.linspace(2.0, 4.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
q_factor = torch.linspace(10, 20, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
h98 = torch.linspace(1.3, 1.6, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

sparc_sym = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

real_data = torch.cat([b_t, i_p, beta_n, q_factor, h98, sparc_sym], dim=-1)\
...
# Sectors: B_t, I_p, β_N, Q factor, H98, SPARC symmetry, Prediction nulling
...
class E8SPARCFusionReactor(nn.Module):
...
# Initial SPARC reactor state → precision target
states = real_data
target_prec = torch.ones(batch_size, 1, device=device)

model = E8SPARCFusionReactor().to(device)
...
var_data = torch.cat([b_t, i_p, beta_n_var, q_factor, h98, sparc_sym], dim=-1)\
             .repeat(1, 1, dim // 6) * torch.randn(batch_size, seq_len, dim, device=device) * noise_scale
...
class AblationModel(E8SPARCFusionReactor):
    def __init__(self, depth=256):
...
plt.tight_layout()
plt.savefig("sparc_reactor_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as sparc_reactor_ablation_precision_entropy.png")
