...
# Real SPARC data proxy (B_t ~12.2 T, I_p ~8.7 MA, β_N ~2.0 per CFS 2025)
# Vary B_t/I_p (SPARC baseline vs ITER 5.3 T/15 MA)
b_t = torch.linspace(5.3, 12.2, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
...
class E8SPARC HighField(nn.Module):
...
model = E8SPARC HighField().to(device)
