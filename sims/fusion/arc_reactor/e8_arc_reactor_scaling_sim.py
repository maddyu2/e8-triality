...
triality = 3
heads = triality  # Fixed undefined heads variable
dim = 240
latent_dim = 8
seq_len = 1024
noise_scale = 0.002
...
# ARC reactor scaling proxies (B_t ~9 T, I_p ~8 MA, β_N ~2.5–3.5, Q >10, confinement H98 ~1.2–1.5)
...
class ARCScalingRotary(nn.Module):
    def __init__(self):
        super().__init__()
...
# Initial ARC scaling state → precision target
states = real_data
target_prec = torch.ones(batch_size, 1, device=device)

model = E8ARCR eactorScaling().to(device)
...
class AblationModel(E8ARCR eactorScaling):
    def __init__(self, depth=256):
...
plt.tight_layout()
plt.savefig("arc_reactor_scaling_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as arc_reactor_scaling_ablation_precision_entropy.png")
