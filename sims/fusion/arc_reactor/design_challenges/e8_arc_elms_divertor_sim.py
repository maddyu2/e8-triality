...
# ARC ELM suppression + divertor detachment proxies (RMP current 1–5 kA, ELM loss <0.5 MJ/m², detachment fraction 0.8–1.0, heat flux <10 MW/m²)
...
heat_flux = torch.linspace(5, 10, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # MW/m²

arc_sym = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

real_data = torch.cat([rmp_current, elm_energy_loss, detach_frac, heat_flux, arc_sym], dim=-1)\
...
# Sectors: RMP current, ELM energy loss, Detachment fraction, Heat flux, ARC symmetry, Prediction nulling
rmp_roots = e8_roots[:48]
elm_roots = e8_roots[48:96]
detach_roots = e8_roots[96:144]
flux_roots = e8_roots[144:192]
sym_roots = e8_roots[192:]

class ARCELMRotary(nn.Module):
    def __init__(self):
        super().__init__()
...
# Initial ARC ELM + divertor state → precision target
states = real_data
target_prec = torch.ones(batch_size, 1, device=device)

model = E8ARCELMSDivertor().to(device)
...
var_data = torch.cat([rmp_current_var, elm_energy_loss, detach_frac, heat_flux, arc_sym], dim=-1)\
             .repeat(1, 1, dim // 5) * torch.randn(batch_size, seq_len, dim, device=device) * noise_scale
...
class AblationModel(E8ARCELMSDivertor):
    def __init__(self, depth=256):
...
plt.tight_layout()
plt.savefig("arc_elms_divertor_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as arc_elms_divertor_ablation_precision_entropy.png")
