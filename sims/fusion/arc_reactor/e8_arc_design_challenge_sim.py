...
# ARC design challenge proxies (B_t ~9 T, R ~3.3 m, β_N ~2.5–3.5, Q >10, heat flux ~10–20 MW/m²)
b_t = torch.linspace(8.5, 9.5, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # T
major_radius = torch.linspace(3.0, 3.6, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # m
beta_n = torch.linspace(2.5, 3.5, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
q_factor = torch.linspace(10, 15, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
heat_flux = torch.linspace(10, 20, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # MW/m²

arc_sym = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

real_data = torch.cat([b_t, major_radius, beta_n, q_factor, heat_flux, arc_sym], dim=-1)\
...
# Sectors: B_t, Major radius, β_N, Q factor, Heat flux, ARC symmetry, Prediction nulling
bt_roots = e8_roots[:40]
radius_roots = e8_roots[40:80]
beta_roots = e8_roots[80:120]
q_roots = e8_roots[120:160]
flux_roots = e8_roots[160:200]
sym_roots = e8_roots[200:]

class ARCDesignRotary(nn.Module):
    def __init__(self):
        super().__init__()
...
# Initial ARC design challenge state → precision target
states = real_data
target_prec = torch.ones(batch_size, 1, device=device)

model = E8ARCDesignChallenge().to(device)
...
# ────────────────────────────────────────────────
# Sensitivity Analysis: Vary heat_flux ±10%
# ────────────────────────────────────────────────

heat_flux_var = heat_flux * torch.linspace(0.9, 1.1, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
var_data = torch.cat([b_t, major_radius, beta_n, q_factor, heat_flux_var, arc_sym], dim=-1)\
             .repeat(1, 1, dim // 6) * torch.randn(batch_size, seq_len, dim, device=device) * noise_scale

var_prec = model(var_data, epochs - 1)
var_ent = -var_prec * torch.log(var_prec + 1e-12)
var_variance = torch.var(var_prec.mean(dim=0)).item()
print(f"Sensitivity variance on precision (heat_flux ±10%): {var_variance:.6f} — low variance = robust")

# ────────────────────────────────────────────────
# Sigma Test: Ablation baseline (no triality)
# ────────────────────────────────────────────────

class AblationModel(E8ARCDesignChallenge):
    def __init__(self, depth=256):
...
plt.tight_layout()
plt.savefig("arc_reactor_design_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as arc_reactor_design_ablation_precision_entropy.png")
