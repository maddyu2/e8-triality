...
# ARC reactor design proxies (high-field compact tokamak: B_t ~9 T, I_p ~8 MA, β_N ~2.5–3.5, Q >10, H98 ~1.2–1.5)
...
# Initial ARC reactor state → precision target
states = real_data
target_prec = torch.ones(batch_size, 1, device=device)

model = E8ARCFusionReactor().to(device)
...
var_data = torch.cat([b_t, i_p, beta_n_var, q_factor, h98, arc_sym], dim=-1)\
             .repeat(1, 1, dim // 6) * torch.randn(batch_size, seq_len, dim, device=device) * noise_scale
...
class AblationModel(E8ARCFusionReactor):
    def __init__(self, depth=256):
...
plt.tight_layout()
plt.savefig("arc_reactor_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as arc_reactor_ablation_precision_entropy.png")
