...
# ARC reactor triality integration proxies (high-field B_t ~9 T, I_p 7–9 MA, β_N 2.5–3.5, Q >10, confinement H98 1.2–1.5)
b_t = torch.linspace(8.5, 9.5, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # T
i_p = torch.linspace(7.0, 9.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # MA
...
# Initial ARC triality integration state → precision target
states = real_data
target_prec = torch.ones(batch_size, 1, device=device)

model = E8ARCR eactorTrialityIntegration().to(device)
...
# ────────────────────────────────────────────────
# Sensitivity Analysis: Vary b_t ±10%
# ────────────────────────────────────────────────

b_t_var = b_t * torch.linspace(0.9, 1.1, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
var_data = torch.cat([b_t_var, i_p, beta_n, q_factor, h98, arc_sym], dim=-1)\
             .repeat(1, 1, dim // 6) * torch.randn(batch_size, seq_len, dim, device=device) * noise_scale

var_prec = model(var_data, epochs - 1)
var_ent = -var_prec * torch.log(var_prec + 1e-12)
var_variance = torch.var(var_prec.mean(dim=0)).item()
print(f"Sensitivity variance on precision (B_t ±10%): {var_variance:.6f} — low variance = robust")

# ────────────────────────────────────────────────
# Sigma Test: Ablation baseline (no triality)
# ────────────────────────────────────────────────

class AblationModel(E8ARCR eactorTrialityIntegration):
    def __init__(self, depth=256):
...
plt.tight_layout()
plt.savefig("arc_reactor_triality_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as arc_reactor_triality_ablation_precision_entropy.png")
