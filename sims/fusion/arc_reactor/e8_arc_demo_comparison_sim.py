...
# ARC vs DEMO proxies (ARC: B_t 8.5–9.5 T, DEMO: B_t 5.5–6.5 T, Q 10–15, β_N 2.5–4.0)
arc_b_t = torch.linspace(8.5, 9.5, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # T
demo_b_t = torch.linspace(5.5, 6.5, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
q_factor = torch.linspace(10, 15, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
beta_n = torch.linspace(2.5, 4.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

comparison_sym = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

real_data = torch.cat([arc_b_t, demo_b_t, q_factor, beta_n, comparison_sym], dim=-1)\
...
# Sectors: ARC B_t, DEMO B_t, Q factor, β_N, Comparison symmetry, Prediction nulling
arc_bt_roots = e8_roots[:48]
demo_bt_roots = e8_roots[48:96]
q_roots = e8_roots[96:144]
beta_roots = e8_roots[144:192]
sym_roots = e8_roots[192:]

class ARCDemoComparisonRotary(nn.Module):
    def __init__(self):
        super().__init__()
...
# Initial ARC vs DEMO state → precision target
states = real_data
target_prec = torch.ones(batch_size, 1, device=device)

model = E8ARCDemoComparison().to(device)
...
# ────────────────────────────────────────────────
# Sensitivity Analysis: Vary q_factor ±10%
# ────────────────────────────────────────────────

q_factor_var = q_factor * torch.linspace(0.9, 1.1, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
var_data = torch.cat([arc_b_t, demo_b_t, q_factor_var, beta_n, comparison_sym], dim=-1)\
             .repeat(1, 1, dim // 5) * torch.randn(batch_size, seq_len, dim, device=device) * noise_scale

var_prec = model(var_data, epochs - 1)
var_ent = -var_prec * torch.log(var_prec + 1e-12)
var_variance = torch.var(var_prec.mean(dim=0)).item()
print(f"Sensitivity variance on precision (Q-factor ±10%): {var_variance:.6f} — low variance = robust")

# ────────────────────────────────────────────────
# Sigma Test: Ablation baseline (no triality)
# ────────────────────────────────────────────────

class AblationModel(E8ARCDemoComparison):
    def __init__(self, depth=256):
...
plt.tight_layout()
plt.savefig("arc_demo_comparison_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as arc_demo_comparison_ablation_precision_entropy.png")
