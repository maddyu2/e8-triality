...
# Proxy real ITER feeds (n_e, η_e, v_zonal, J_bs, mod_grav hint)
def fetch_iter_proxy():
    # Placeholder for real ITER dashboard
    n_e = 1e20     # m^{-3}
    eta_e = 3.0    # ~2-4
    v_zonal = 30.0 # km/s ~10-50
    j_bs = 0.75    # MA/m² ~0.5-1
    mod_grav = 1e-3 # Modified gravity hint param
    return torch.tensor([n_e, eta_e, v_zonal, j_bs, mod_grav], device=device)

iter_vector = fetch_iter_proxy().repeat(batch_size, seq_len, 1)
real_tem_data = iter_vector.repeat(1, 1, dim // 5) * (1 + 0.1 * torch.randn(batch_size, seq_len, dim // 5, device=device))

# Vary v_zonal as vectors
