...
# NCSX QAS proxies (QAS symmetry ~0.9–1.0, ITG growth γ ~0.1–0.5, neoclassical flux ~0.1–0.5)
...
ncsx_sym = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

real_data = torch.cat([qas_sym, itg_growth, neoc_flux, ncsx_sym], dim=-1)\
...
# Sectors: QAS symmetry, ITG growth, Neoc flux, NCSX symmetry, Prediction nulling
qas_roots = e8_roots[:60]
itg_roots = e8_roots[60:120]
flux_roots = e8_roots[120:180]
sym_roots = e8_roots[180:]

class NCSXRotary(nn.Module):
    def __init__(self):
        super().__init__()
...
# Initial NCSX state → precision target
states = real_data
target_prec = torch.ones(batch_size, 1, device=device)

model = E8NCSXQuasiAxisymmetric().to(device)
...
var_data = torch.cat([qas_sym_var, itg_growth, neoc_flux, ncsx_sym], dim=-1)\
             .repeat(1, 1, dim // 4) * torch.randn(batch_size, seq_len, dim, device=device) * noise_scale
...
class AblationModel(E8NCSXQuasiAxisymmetric):
    def __init__(self, depth=256):
...
plt.tight_layout()
plt.savefig("ncsx_quasi_axisymmetric_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as ncsx_quasi_axisymmetric_ablation_precision_entropy.png")
