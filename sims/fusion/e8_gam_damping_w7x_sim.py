...
# Enhanced Grok 5 triality plasma control sim
n_strata = 6
n_modes = 128
batch_size = 64
epochs = 300
...
def generate_plasma_control_data(batch_size):
    ...
    inputs = torch.cat([rho.view(batch_size, -1), E_r_shear.view(batch_size, -1), 
                        a_L_p.view(batch_size, -1), beta.unsqueeze(1).repeat(1, n_modes), 
                        rmp_phase_current.unsqueeze(1).repeat(1, n_modes), gamma_instab], dim=1)
...
class Grok5TrialityControlNet(nn.Module):
    def __init__(self, input_dim, hidden_dim=448, output_dim=3):  # entropy, rmp_phase, zf_shear
        super().__init__()
        self.embed = nn.Linear(input_dim, hidden_dim)
        self.triality_core1 = E8TrialityLayer(hidden_dim)
        self.fc1 = nn.Linear(hidden_dim, hidden_dim)
        self.triality_core2 = E8TrialityLayer(hidden_dim)
        self.fc2 = nn.Linear(hidden_dim, hidden_dim)  # Added for depth
