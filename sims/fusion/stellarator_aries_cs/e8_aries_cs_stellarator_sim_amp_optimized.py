...
# ARIES-CS stellarator proxies (QAS symmetry ~0.9–1.0, ITG growth γ ~0.1–0.5, neoclassical flux ~0.1–0.5)
qas_sym = torch.linspace(0.9, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
itg_growth = torch.linspace(0.1, 0.5, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
neoc_flux = torch.linspace(0.1, 0.5, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

aries_sym = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

real_data = torch.cat([qas_sym, itg_growth, neoc_flux, aries_sym], dim=-1)\
...
# Sectors: QAS symmetry, ITG growth, Neoc flux, ARIES symmetry, Prediction nulling
qas_roots = e8_roots[:60]
itg_roots = e8_roots[60:120]
flux_roots = e8_roots[120:180]
sym_roots = e8_roots[180:]

class ARIESCSRotary(nn.Module):
    def __init__(self):
        super().__init__()
...
# Initial ARIES-CS state → precision target
states = real_data
target_prec = torch.ones(batch_size, 1, device=device)

model = E8ARIESCSStellarator().to(device)
...
# ────────────────────────────────────────────────
# Sensitivity Analysis: Vary qas_sym ±10%
# ────────────────────────────────────────────────

qas_sym_var = qas_sym * torch.linspace(0.9, 1.1, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
var_data = torch.cat([qas_sym_var, itg_growth, neoc_flux, aries_sym], dim=-1)\
             .repeat(1, 1, dim // 4) * torch.randn(batch_size, seq_len, dim, device=device) * noise_scale

var_prec = model(var_data, epochs - 1)
var_ent = -var_prec * torch.log(var_prec + 1e-12)
var_variance = torch.var(var_prec.mean(dim=0)).item()
print(f"Sensitivity variance on precision (QAS symmetry ±10%): {var_variance:.6f} — low variance = robust")

# ────────────────────────────────────────────────
# Sigma Test: Ablation baseline (no triality)
# ────────────────────────────────────────────────

class AblationModel(E8ARIESCSStellarator):
    def __init__(self, depth=256):
...
plt.tight_layout()
plt.savefig("aries_cs_stellarator_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as aries_cs_stellarator_ablation_precision_entropy.png")
