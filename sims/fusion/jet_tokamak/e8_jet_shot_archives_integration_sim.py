...
# JET shot archive proxies (high-performance ILW pulses, 2020–2025 data ranges)
# Core Te 8–12 keV, ne 8–12×10^{19} m^{-3}, β_N 2.0–3.0, H98(y,2) 1.0–1.2, radiation fraction 60–80%
...
real_jet_data = torch.cat([te_core, ne_core, beta_n_jet, h98_factor, rad_fraction, jet_sym], dim=-1)\
...
# Sectors: Te core, ne core, β_N JET, H98 factor, Radiation fraction, JET symmetry, Prediction nulling
...
# Initial JET shot archives state → precision target
states = real_jet_data
target_prec = torch.ones(batch_size, 1, device=device)

model = E8JETShotArchivesIntegration().to(device)
...
var_data = torch.cat([te_core_var, ne_core, beta_n_jet, h98_factor, rad_fraction, jet_sym], dim=-1)\
             .repeat(1, 1, dim // 6) * torch.randn(batch_size, seq_len, dim, device=device) * noise_scale
...
class AblationModel(E8JETShotArchivesIntegration):
    def __init__(self, depth=256):
...
plt.tight_layout()
plt.savefig("jet_shot_archives_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as jet_shot_archives_ablation_precision_entropy.png")
