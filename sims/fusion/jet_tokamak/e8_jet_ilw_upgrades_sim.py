...
# JET ILW upgrades proxies (post-2011 ITER-like wall: Te core 8–12 keV, ne 8–12×10^{19} m^{-3}, β_N 2.0–3.0, H98(y,2) 1.0–1.2, radiation fraction >60%)
...
h98 = torch.linspace(1.0, 1.2, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
rad_frac = torch.linspace(0.6, 0.8, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

jet_ilw_sym = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

real_data = torch.cat([te_core, ne_core, beta_n, h98, rad_frac, jet_ilw_sym], dim=-1)\
...
# Sectors: Te core, ne core, β_N, H98, Radiation fraction, JET-ILW symmetry, Prediction nulling
te_roots = e8_roots[:40]
ne_roots = e8_roots[40:80]
beta_roots = e8_roots[80:120]
h98_roots = e8_roots[120:160]
rad_roots = e8_roots[160:200]
...
class E8JETILWUpgrades(nn.Module):
...
# Initial JET-ILW state → precision target
states = real_data
target_prec = torch.ones(batch_size, 1, device=device)

model = E8JETILWUpgrades().to(device)
...
var_data = torch.cat([te_core_var, ne_core, beta_n_jet, h98_factor, jet_ilw_sym], dim=-1)\
...
class AblationModel(E8JETILWUpgrades):
    def __init__(self, depth=256):
...
plt.tight_layout()
plt.savefig("jet_ilw_upgrades_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as jet_ilw_upgrades_ablation_precision_entropy.png")
