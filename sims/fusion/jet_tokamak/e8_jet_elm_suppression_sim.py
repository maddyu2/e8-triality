...
# JET ELM suppression proxies (RMP current ~1–4 kA, pellet pacing 20–80 Hz, suppression efficiency 0.7–1.0)
rmp_current = torch.linspace(1, 4, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # kA
pellet_rate = torch.linspace(20, 80, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # Hz
...
class E8JETELMSuppression(nn.Module):
...
# Initial JET ELM suppression state → precision target
states = real_data
target_prec = torch.ones(batch_size, 1, device=device)

model = E8JETELMSuppression().to(device)
...
class AblationModel(E8JETELMSuppression):
    def __init__(self, depth=256):
...
plt.tight_layout()
plt.savefig("jet_elm_suppression_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as jet_elm_suppression_ablation_precision_entropy.png")
