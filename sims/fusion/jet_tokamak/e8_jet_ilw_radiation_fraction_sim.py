...
# JET ILW radiation fraction proxies (radiation fraction >60%, Te core 8–12 keV, ne 8–12×10^{19} m^{-3}, β_N 2.0–3.0)
rad_fraction = torch.linspace(0.6, 0.85, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
...
ilw_sym = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

real_data = torch.cat([rad_fraction, te_core, ne_core, beta_n, ilw_sym], dim=-1)\
...
# Sectors: Radiation fraction, Te core, ne core, β_N, ILW symmetry, Prediction nulling
rad_roots = e8_roots[:48]
te_roots = e8_roots[48:96]
ne_roots = e8_roots[96:144]
beta_roots = e8_roots[144:192]
sym_roots = e8_roots[192:]

class JetILWRotary(nn.Module):
    def __init__(self):
        super().__init__()
...
# Initial JET-ILW radiation fraction state → precision target
states = real_data
target_prec = torch.ones(batch_size, 1, device=device)

model = E8JETILWRadiationFraction().to(device)
...
# ────────────────────────────────────────────────
# Sensitivity Analysis: Vary rad_fraction ±10%
# ────────────────────────────────────────────────

rad_fraction_var = rad_fraction * torch.linspace(0.9, 1.1, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
var_data = torch.cat([rad_fraction_var, te_core, ne_core, beta_n, ilw_sym], dim=-1)\
             .repeat(1, 1, dim // 5) * torch.randn(batch_size, seq_len, dim, device=device) * noise_scale

var_prec = model(var_data, epochs - 1)
var_ent = -var_prec * torch.log(var_prec + 1e-12)
var_variance = torch.var(var_prec.mean(dim=0)).item()
print(f"Sensitivity variance on precision (rad_fraction ±10%): {var_variance:.6f} — low variance = robust")

# ────────────────────────────────────────────────
# Sigma Test: Ablation baseline (no triality)
# ────────────────────────────────────────────────

class AblationModel(E8JETILWRadiationFraction):
    def __init__(self, depth=256):
...
plt.tight_layout()
plt.savefig("jet_ilw_radiation_fraction_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as jet_ilw_radiation_fraction_ablation_precision_entropy.png")
