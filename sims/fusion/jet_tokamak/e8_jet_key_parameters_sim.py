...
# JET key parameters proxies (2025 high-performance ILW pulses)
# Core Te 8–12 keV, ne 8–12×10^{19} m^{-3}, β_N 2.0–3.0, H98(y,2) 1.0–1.2, I_p 2.0–3.0 MA
...
i_p = torch.linspace(2.0, 3.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # MA

jet_sym = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

real_jet_data = torch.cat([te_core, ne_core, beta_n, h98, i_p, jet_sym], dim=-1)\
...
# Sectors: Te core, ne core, β_N, H98, I_p, JET symmetry, Prediction nulling
te_roots = e8_roots[:40]
ne_roots = e8_roots[40:80]
beta_roots = e8_roots[80:120]
h98_roots = e8_roots[120:160]
ip_roots = e8_roots[160:200]
sym_roots = e8_roots[200:]

class JetKeyRotary(nn.Module):
    def __init__(self):
        super().__init__()
...
# Initial JET key parameters state → precision target
states = real_jet_data
target_prec = torch.ones(batch_size, 1, device=device)

model = E8JETKeyParameters().to(device)
...
var_data = torch.cat([te_core_var, ne_core, beta_n_jet, h98_factor, i_p, jet_sym], dim=-1)\
             .repeat(1, 1, dim // 6) * torch.randn(batch_size, seq_len, dim, device=device) * noise_scale
...
class AblationModel(E8JETKeyParameters):
    def __init__(self, depth=256):
...
plt.tight_layout()
plt.savefig("jet_key_parameters_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as jet_key_parameters_ablation_precision_entropy.png")
