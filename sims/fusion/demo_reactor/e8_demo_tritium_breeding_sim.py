...
# DEMO tritium breeding proxies (TBR >1.05, neutron flux ~10^{18} n/m²s, Li-6 enrichment 60–90%)
tbr = torch.linspace(1.0, 1.2, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
neutron_flux = torch.linspace(1e17, 1e19, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # n/m²s
li6_enrich = torch.linspace(0.6, 0.9, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

breeding_sym = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

real_data = torch.cat([tbr, neutron_flux, li6_enrich, breeding_sym], dim=-1)\
...
# Sectors: TBR, Neutron flux, Li-6 enrich, Breeding symmetry, Prediction nulling
tbr_roots = e8_roots[:60]
flux_roots = e8_roots[60:120]
li6_roots = e8_roots[120:180]
sym_roots = e8_roots[180:]

class TritiumBreedingRotary(nn.Module):
    def __init__(self):
        super().__init__()
...
model = E8DEMO TritiumBreeding().to(device)
...
# ────────────────────────────────────────────────
# Sensitivity Analysis: Vary tbr ±10%
# ────────────────────────────────────────────────

tbr_var = tbr * torch.linspace(0.9, 1.1, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
var_data = torch.cat([tbr_var, neutron_flux, li6_enrich, breeding_sym], dim=-1)\
             .repeat(1, 1, dim // 4) * torch.randn(batch_size, seq_len, dim, device=device) * noise_scale

var_prec = model(var_data, epochs - 1)
var_ent = -var_prec * torch.log(var_prec + 1e-12)
var_variance = torch.var(var_prec.mean(dim=0)).item()
print(f"Sensitivity variance on precision (TBR ±10%): {var_variance:.6f} — low variance = robust")

# ────────────────────────────────────────────────
# Sigma Test: Ablation baseline (no triality)
# ────────────────────────────────────────────────

class AblationModel(E8DEMOTritiumBreeding):
    def __init__(self, depth=256):
...
plt.tight_layout()
plt.savefig("demo_tritium_breeding_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as demo_tritium_breeding_ablation_precision_entropy.png")
