...
# DEMO core stability proxies (β_N ~3.0–4.0, q-profile 2.0–4.0, stability margin 0.9–1.0)
beta_n = torch.linspace(3.0, 4.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
q_profile = torch.linspace(2.0, 4.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
stability_margin = torch.linspace(0.9, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

core_sym = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

real_data = torch.cat([beta_n, q_profile, stability_margin, core_sym], dim=-1)\
...
# Sectors: β_N, q-profile, Stability margin, Core symmetry, Prediction nulling
beta_roots = e8_roots[:60]
q_roots = e8_roots[60:120]
margin_roots = e8_roots[120:180]
sym_roots = e8_roots[180:]

class CoreStabilityRotary(nn.Module):
    def __init__(self):
        super().__init__()
...
# Initial DEMO core state → precision target
states = real_data
target_prec = torch.ones(batch_size, 1, device=device)

model = E8DEMOCoreStability().to(device)
...
var_data = torch.cat([beta_n_var, q_profile, stability_margin, core_sym], dim=-1)\
             .repeat(1, 1, dim // 4) * torch.randn(batch_size, seq_len, dim, device=device) * noise_scale
...
class AblationModel(E8DEMOCoreStability):
    def __init__(self, depth=256):
...
plt.tight_layout()
plt.savefig("demo_core_stability_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as demo_core_stability_ablation_precision_entropy.png")
