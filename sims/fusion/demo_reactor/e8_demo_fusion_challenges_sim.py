...
# DEMO fusion challenges proxies (high β_N ~3.0–4.0, TBR >1.05, pulse length ~2–10 hours, heat flux ~10–20 MW/m²)
beta_n = torch.linspace(3.0, 4.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
tbr = torch.linspace(1.0, 1.2, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
pulse_length = torch.linspace(2, 10, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # hours
...
real_data = torch.cat([beta_n, tbr, pulse_length, heat_flux, demo_sym], dim=-1)\
...
# Sectors: β_N, TBR, Pulse length, Heat flux, DEMO symmetry, Prediction nulling
beta_roots = e8_roots[:48]
tbr_roots = e8_roots[48:96]
pulse_roots = e8_roots[96:144]
flux_roots = e8_roots[144:192]
sym_roots = e8_roots[192:]

class DemoChallengesRotary(nn.Module):
    def __init__(self):
        super().__init__()
...
# Initial DEMO challenges state → precision target
states = real_data
target_prec = torch.ones(batch_size, 1, device=device)

model = E8DEMOFusionChallenges().to(device)
...
var_data = torch.cat([beta_n_var, tbr, pulse_length, heat_flux, demo_sym], dim=-1)\
             .repeat(1, 1, dim // 5) * torch.randn(batch_size, seq_len, dim, device=device) * noise_scale
...
class AblationModel(E8DEMOFusionChallenges):
    def __init__(self, depth=256):
...
plt.tight_layout()
plt.savefig("demo_fusion_challenges_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as demo_fusion_challenges_ablation_precision_entropy.png")
