...
# DEMO reactor scaling proxies (B_t ~6 T, I_p ~19 MA, β_N ~3.0–4.0, P_fus ~2 GW, τ_E ~4–8 s)
...
var_data = torch.cat([b_t, i_p, beta_n_var, p_fus, tau_e, demo_sym], dim=-1)\
             .repeat(1, 1, dim // 6) * torch.randn(batch_size, seq_len, dim, device=device) * noise_scale
...
class AblationModel(E8DEMOR eactorScaling):
    def __init__(self, depth=256):
