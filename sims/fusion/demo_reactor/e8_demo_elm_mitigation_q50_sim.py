...
# DEMO ELM mitigation + Q>50 proxies (2025–2030 targets)
# RMP current 1–6 kA, pedestal pressure 150–300 kPa, ELM loss <0.3 MJ/m²
# Fusion power 2–3 GW, confinement time 4–8 s → Q>50
rmp_current = torch.linspace(1, 6, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # kA
ped_pressure = torch.linspace(150, 300, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # kPa
elm_loss = torch.linspace(0.05, 0.3, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # MJ/m²
fusion_power = torch.linspace(2000, 3000, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # MW
conf_time = torch.linspace(4, 8, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # s

demo_sym = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

real_data = torch.cat([rmp_current, ped_pressure, elm_loss, fusion_power, conf_time, demo_sym], dim=-1)\
...
# Sectors: RMP current, Pedestal pressure, ELM loss, Fusion power, Confinement time, DEMO symmetry
rmp_roots = e8_roots[:40]
press_roots = e8_roots[40:80]
elm_roots = e8_roots[80:120]
power_roots = e8_roots[120:160]
time_roots = e8_roots[160:200]
sym_roots = e8_roots[200:]

class ELMMitigationRotary(nn.Module):
    def __init__(self):
        super().__init__()
...
# Initial state → target precision
states = real_data
target_prec = torch.ones(batch_size, 1, device=device)

model = E8DEMOELMMitigationQ50().to(device)
...
# ────────────────────────────────────────────────
# Sensitivity Analysis: Vary rmp_current ±10%
# ────────────────────────────────────────────────

rmp_current_var = rmp_current * torch.linspace(0.9, 1.1, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
var_data = torch.cat([rmp_current_var, ped_pressure, elm_energy_loss, fusion_power, conf_time, demo_sym], dim=-1)\
             .repeat(1, 1, dim // 6) * torch.randn(batch_size, seq_len, dim, device=device) * noise_scale

var_prec = model(var_data, epochs - 1)
var_ent = -var_prec * torch.log(var_prec + 1e-12)
var_variance = torch.var(var_prec.mean(dim=0)).item()
print(f"Sensitivity variance on precision (RMP current ±10%): {var_variance:.6f} — low variance = robust")

# ────────────────────────────────────────────────
# Sigma Test: Ablation baseline (no triality)
# ────────────────────────────────────────────────

class AblationModel(E8DEMOELMMitigationQ50):
    def __init__(self, depth=256):
...
plt.tight_layout()
plt.savefig("demo_elm_mitigation_q50_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as demo_elm_mitigation_q50_ablation_precision_entropy.png")
