...
# EUROfusion DEMO 2025 baseline (B_t ~6.3 T, I_p ~19 MA, β_N ~2.0-3.0, net 500 MW(e))
# Vary B_t/I_p/β_N as input vectors
b_t = torch.linspace(6.0, 6.6, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # T
...
class E8DEMOReactor(nn.Module):
    def __init__(self, depth=256):  # Scaled depth
        super().__init__()
        subsets = [bt_roots, ip_roots, betan_roots, sym_roots]
