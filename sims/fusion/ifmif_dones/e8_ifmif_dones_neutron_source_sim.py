...
# IFMIF-DONES neutron source proxies (2025–2030 targets)
# Neutron flux ~10^{17}–10^{18} n/m²s, deuteron energy 40 MeV, Li target thickness ~1–2 cm, irradiation volume ~0.5–1 liter
neutron_flux = torch.linspace(1e17, 1e18, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # n/m²s
...
irradiation_volume = torch.linspace(0.5, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # liters

ifmif_sym = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

real_data = torch.cat([neutron_flux, deuteron_energy, li_thickness, irradiation_volume, ifmif_sym], dim=-1)\
...
# Sectors: Neutron flux, Deuteron energy, Li thickness, Irradiation volume, IFMIF symmetry, Prediction nulling
flux_roots = e8_roots[:48]
energy_roots = e8_roots[48:96]
thickness_roots = e8_roots[96:144]
volume_roots = e8_roots[144:192]
sym_roots = e8_roots[192:]

class IFMIFRotary(nn.Module):
    def __init__(self):
        super().__init__()
...
# Initial IFMIF-DONES state → precision target
states = real_data
target_prec = torch.ones(batch_size, 1, device=device)

model = E8IFMIFDONESNeutronSource().to(device)
...
var_data = torch.cat([neutron_flux_var, deuteron_energy, li_thickness, irradiation_volume, ifmif_sym], dim=-1)\
             .repeat(1, 1, dim // 5) * torch.randn(batch_size, seq_len, dim, device=device) * noise_scale
...
class AblationModel(E8IFMIFDONESNeutronSource):
    def __init__(self, depth=256):
...
plt.tight_layout()
plt.savefig("ifmif_dones_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as ifmif_dones_ablation_precision_entropy.png")
