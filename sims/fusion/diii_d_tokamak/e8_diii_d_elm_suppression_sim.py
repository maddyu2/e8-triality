...
# DIII-D ELM suppression proxies (RMP current 1–5 kA, pedestal gradient 1–10, ELM energy loss <0.5 MJ/m²)
...
real_data = torch.cat([rmp_current, ped_gradient, elm_energy_loss, elm_sym], dim=-1)\
...
# Sectors: RMP current, Ped gradient, ELM energy loss, ELM symmetry, Prediction nulling
...
class E8DIIIDELMSuppression(nn.Module):
...
# Initial DIII-D ELM suppression state → precision target
states = real_data
target_prec = torch.ones(batch_size, 1, device=device)

model = E8DIIIDELMSuppression().to(device)
...
var_data = torch.cat([rmp_current_var, ped_gradient, elm_energy_loss, elm_sym], dim=-1)\
             .repeat(1, 1, dim // 4) * torch.randn(batch_size, seq_len, dim, device=device) * noise_scale
...
class AblationModel(E8DIIIDELMSuppression):
    def __init__(self, depth=256):
