...
# DIII-D key parameters proxies (B_t 1.5–2.1 T, I_p 0.8–2.0 MA, β_N up to 4.0, H98(y,2) 1.0–1.5, Te core 5–15 keV)
b_t = torch.linspace(1.5, 2.1, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # T
i_p = torch.linspace(0.8, 2.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # MA
beta_n = torch.linspace(2.0, 4.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
h98 = torch.linspace(1.0, 1.5, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
te_core = torch.linspace(5, 15, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # keV

diii_sym = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

real_data = torch.cat([b_t, i_p, beta_n, h98, te_core, diii_sym], dim=-1)\
             .repeat(1, 1, dim // 6) * torch.randn(batch_size, seq_len, dim, device=device) * noise_scale

# E8 roots (same function as above)

# Sectors: B_t, I_p, β_N, H98, Te core, DIII-D symmetry, Prediction nulling
bt_roots = e8_roots[:40]
ip_roots = e8_roots[40:80]
beta_roots = e8_roots[80:120]
h98_roots = e8_roots[120:160]
te_roots = e8_roots[160:200]
sym_roots = e8_roots[200:]

class DIIIDKeyRotary(nn.Module):
    def __init__(self):
        super().__init__()
...
# Initial DIII-D key parameters state → precision target
states = real_data
target_prec = torch.ones(batch_size, 1, device=device)

model = E8DIIIDKeyParameters().to(device)
...
# ────────────────────────────────────────────────
# Sensitivity Analysis: Vary beta_n ±10%
# ────────────────────────────────────────────────

beta_n_var = beta_n * torch.linspace(0.9, 1.1, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
var_data = torch.cat([b_t, i_p, beta_n_var, h98, te_core, diii_sym], dim=-1)\
             .repeat(1, 1, dim // 6) * torch.randn(batch_size, seq_len, dim, device=device) * noise_scale

var_prec = model(var_data, epochs - 1)
var_ent = -var_prec * torch.log(var_prec + 1e-12)
var_variance = torch.var(var_prec.mean(dim=0)).item()
print(f"Sensitivity variance on precision (beta_n ±10%): {var_variance:.6f} — low variance = robust")

# ────────────────────────────────────────────────
# Sigma Test: Ablation baseline (no triality)
# ────────────────────────────────────────────────

class AblationModel(E8DIIIDKeyParameters):
    def __init__(self, depth=256):
...
plt.tight_layout()
plt.savefig("diii_d_key_parameters_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as diii_d_key_parameters_ablation_precision_entropy.png")
