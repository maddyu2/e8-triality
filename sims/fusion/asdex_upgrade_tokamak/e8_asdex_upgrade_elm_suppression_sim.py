...
# ASDEX Upgrade ELM suppression proxies (RMP current 1–4 kA, pellet rate 10–40 Hz, suppression efficiency 0.7–1.0)
rmp_current = torch.linspace(1, 4, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # kA
pellet_rate = torch.linspace(10, 40, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # Hz
supp_eff = torch.linspace(0.7, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

elm_sym = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

real_data = torch.cat([rmp_current, pellet_rate, supp_eff, elm_sym], dim=-1)\
...
# Sectors: RMP current, Pellet rate, Suppression eff, ELM symmetry, Prediction nulling
rmp_roots = e8_roots[:60]
pellet_roots = e8_roots[60:120]
eff_roots = e8_roots[120:180]
sym_roots = e8_roots[180:]

class ASDEXELMRotary(nn.Module):
    def __init__(self):
        super().__init__()
...
# Initial ASDEX ELM suppression state → precision target
states = real_data
target_prec = torch.ones(batch_size, 1, device=device)

model = E8ASDEXUpgradeELMSuppression().to(device)
...
var_data = torch.cat([rmp_current_var, pellet_rate, supp_eff, elm_sym], dim=-1)\
             .repeat(1, 1, dim // 4) * torch.randn(batch_size, seq_len, dim, device=device) * noise_scale

var_prec = model(var_data, epochs - 1)
var_ent = -var_prec * torch.log(var_prec + 1e-12)
var_variance = torch.var(var_prec.mean(dim=0)).item()
print(f"Sensitivity variance on precision (rmp_current ±10%): {var_variance:.6f} — low variance = robust")

# ────────────────────────────────────────────────
# Sigma Test: Ablation baseline (no triality)
# ────────────────────────────────────────────────

class AblationModel(E8ASDEXUpgradeELMSuppression):
    def __init__(self, depth=256):
...
plt.tight_layout()
plt.savefig("diii_d_elm_suppression_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as diii_d_elm_suppression_ablation_precision_entropy.png")
