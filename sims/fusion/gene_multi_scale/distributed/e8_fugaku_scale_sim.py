...
# Fugaku scaling proxies (nodes ~10^4–10^6, Blackwell FP4 efficiency 0.5–2.0, scale factor 1–10)
...
fugaku_sym = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

real_data = torch.cat([fugaku_nodes, blackwell_fp4_eff, scale_factor, fugaku_sym], dim=-1)\
             .repeat(1, 1, dim // 4) * torch.randn(batch_size, seq_len, dim, device=device) * noise_scale

# E8 roots (same function)

# Sectors: Fugaku nodes, Blackwell FP4 eff, Scale factor, Fugaku symmetry, Prediction nulling
nodes_roots = e8_roots[:60]
fp4_roots = e8_roots[60:120]
scale_roots = e8_roots[120:180]
sym_roots = e8_roots[180:]

class FugakuScaleRotary(nn.Module):
    def __init__(self):
        super().__init__()
...
# Initial Fugaku scaling state → precision target
states = real_data
target_prec = torch.ones(batch_size, 1, device=device)

model = E8FugakuScale().to(device)
...
# ────────────────────────────────────────────────
# Sensitivity Analysis: Vary fugaku_nodes ±10%
# ────────────────────────────────────────────────

fugaku_nodes_var = fugaku_nodes * torch.linspace(0.9, 1.1, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
var_data = torch.cat([fugaku_nodes_var, blackwell_fp4_eff, scale_factor, fugaku_sym], dim=-1)\
             .repeat(1, 1, dim // 4) * torch.randn(batch_size, seq_len, dim, device=device) * noise_scale

var_prec = model(var_data, epochs - 1)
var_ent = -var_prec * torch.log(var_prec + 1e-12)
var_variance = torch.var(var_prec.mean(dim=0)).item()
print(f"Sensitivity variance on precision (fugaku_nodes ±10%): {var_variance:.6f} — low variance = robust")

# ────────────────────────────────────────────────
# Sigma Test: Ablation baseline (no triality)
# ────────────────────────────────────────────────

class AblationModel(E8FugakuScale):
    def __init__(self, depth=256):
...
plt.tight_layout()
plt.savefig("fugaku_scale_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as fugaku_scale_ablation_precision_entropy.png")
