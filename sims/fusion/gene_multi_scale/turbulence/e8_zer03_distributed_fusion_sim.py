...
# DDP Setup (required for multi-GPU / Colossus)
...
grad_accum_steps = 4       # effective batch = 1024 × 4 = 4096
epochs = 3000000

# ────────────────────────────────────────────────
# Data Generation (multi-scale ITER turbulence)
# ────────────────────────────────────────────────
...
# DeepSpeed ZeRO-3 config (offload optimizer states & gradients to CPU)
ds_config = {
    "zero_optimization": {
        "stage": 3,
        "offload_optimizer": {
...
if local_rank == 0:
    ...
    plt.tight_layout()
    plt.savefig("zer03_multi_scale_precision_entropy.png", dpi=300, bbox_inches='tight')
    plt.show()

    print("Plots saved as zer03_multi_scale_precision_entropy.png")
