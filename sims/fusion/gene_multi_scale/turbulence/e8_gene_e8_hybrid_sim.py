...
# GENE surrogate time (normalized runtime) + E8 entropy gain (reduction factor)
gene_surrogate_time = torch.linspace(0.1, 10.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
e8_entropy_gain     = torch.linspace(0.5, 2.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
hybrid_sym          = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

real_data = torch.cat([gene_surrogate_time, e8_entropy_gain, hybrid_sym], dim=-1)\
...
class E8GeneE8Hybrid(nn.Module):
...
model = E8GeneE8Hybrid().to(device)
...
plt.tight_layout()
plt.savefig("gene_e8_hybrid_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Saved: gene_e8_hybrid_precision_entropy.png")
