...
# 2023 studies proxies for phonon data (BSCCO/YBCO 200-600 cm^{-1}, LK-99 100-500 cm^{-1})
phonon = torch.linspace(100, 600, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # cm^{-1}

# Vary Tc (BSCCO/YBCO ~93K vs LK-99 claimed 400K)
tc = torch.linspace(93, 400, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # K

# Electron pairing symmetry proxy (d-wave, energy ~30-50 meV for BSCCO)
pairing = torch.linspace(30, 50, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # meV

real_bscco_data = torch.cat([phonon, tc, pairing], dim=-1).repeat(1, 1, dim // 3) * torch.randn(batch_size, seq_len, dim, device=device) * 0.01
