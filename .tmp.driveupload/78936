...
seq_len = 1024
noise_scale = 0.002
batch_size = 128  # Increased for faster convergence

# IPCC AR7 2025 proxies for SAI data (altitude 10-15 km for CCT, cooling 0.5-1°C, ice nuclei injection 5-10 Tg/yr)
# Vary altitude/cooling (CCT 10-15 km/0.5-1°C vs SAI 20-25 km/1-5°C)
altitude = torch.linspace(10, 15, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # km
cooling = torch.linspace(0.5, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # °C
...
# Injection proxy (ice nuclei 5-10 Tg/yr)
...
opt = torch.optim.AdamW(model.parameters(), lr=1e-4, weight_decay=1e-10)  # Increased lr for faster convergence
...
with torch.autocast(device_type='cuda' if 'cuda' in device else 'cpu'):
    for epoch in range(3000000):
        ...
        loss.backward()
        torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)  # Adjusted clip for stability
        opt.step()
        scheduler.step()
        if epoch % 750000 == 0:
