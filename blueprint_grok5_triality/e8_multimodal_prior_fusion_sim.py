...
# Sparse multimodal proxies (40–70% missing modalities, coherence target 0.85–1.0)
missing_modalities = torch.linspace(0.4, 0.7, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
text_emb = torch.linspace(0.5, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # proxy
image_emb = torch.linspace(0.5, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # proxy
video_emb = torch.linspace(0.5, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # proxy

fusion_sym = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

real_data = torch.cat([missing_modalities, text_emb, image_emb, video_emb, fusion_sym], dim=-1)\
...
# Sectors: Missing modalities, Text emb, Image emb, Video emb, Fusion symmetry, Prediction nulling
missing_roots = e8_roots[:48]
text_roots = e8_roots[48:96]
image_roots = e8_roots[96:144]
video_roots = e8_roots[144:192]
sym_roots = e8_roots[192:]

class MultimodalPriorRotary(nn.Module):
    def __init__(self):
        super().__init__()
...
# Initial multimodal fusion state → precision target
states = real_data
target_prec = torch.ones(batch_size, 1, device=device)

model = E8MultimodalPriorFusion().to(device)
...
# ────────────────────────────────────────────────
# Sensitivity Analysis: Vary missing_modalities ±10%
# ────────────────────────────────────────────────

missing_modalities_var = missing_modalities * torch.linspace(0.9, 1.1, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
var_data = torch.cat([missing_modalities_var, text_emb, image_emb, video_emb, fusion_sym], dim=-1)\
             .repeat(1, 1, dim // 5) * torch.randn(batch_size, seq_len, dim, device=device) * noise_scale

var_prec = model(var_data, epochs - 1)
var_ent = -var_prec * torch.log(var_prec + 1e-12)
var_variance = torch.var(var_prec.mean(dim=0)).item()
print(f"Sensitivity variance on precision (missing modalities ±10%): {var_variance:.6f} — low variance = robust")

# ────────────────────────────────────────────────
# Sigma Test: Ablation baseline (no triality)
# ────────────────────────────────────────────────

class AblationModel(E8MultimodalPriorFusion):
    def __init__(self, depth=256):
...
plt.tight_layout()
plt.savefig("multimodal_prior_fusion_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as multimodal_prior_fusion_ablation_precision_entropy.png")
