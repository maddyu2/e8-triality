...
original_wave = real_data[:, :, 0].cpu().numpy()  # take first channel as waveform proxy
recon_wave = reconstructed[:, 0].cpu().numpy()  # reconstructed scalar proxy (simplified)

# Compute spectrograms
original_spec = []
recon_spec = []
for i in range(min(8, batch_size)):  # visualize first 8
    orig_wave = torch.from_numpy(original_wave[i]).to(device)
    recon_wave_t = torch.from_numpy(recon_wave[i]).to(device)

    orig_stft = torch.stft(orig_wave, n_fft=n_fft, hop_length=hop_length, return_complex=True).abs().cpu().numpy()
    recon_stft = torch.stft(recon_wave_t, n_fft=n_fft, hop_length=hop_length, return_complex=True).abs().cpu().numpy()

    original_spec.append(orig_stft)
    recon_spec.append(recon_stft)
...
for i in range(min(8, batch_size)):
    axes[0, i].imshow(original_spec[i], aspect='auto', origin='lower', cmap='viridis')
    axes[0, i].set_title(f"Masked Audio #{i+1}")
    axes[0, i].axis('off')

    axes[1, i].imshow(recon_spec[i], aspect='auto', origin='lower', cmap='viridis')
    axes[1, i].set_title(f"Reconstructed #{i+1}")
    axes[1, i].axis('off')

plt.suptitle("Original Masked vs Triality Reconstructed Spectrograms")
plt.tight_layout()
plt.savefig("audio_fusion_spectrogram_comparison.png", dpi=300, bbox_inches='tight')
plt.show()

print("Spectrogram visualization saved as audio_fusion_spectrogram_comparison.png")
