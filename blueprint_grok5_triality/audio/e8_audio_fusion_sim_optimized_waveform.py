...
# ────────────────────────────────────────────────
# Synthetic audio waveform proxy (sine wave + noise + masking)
# ────────────────────────────────────────────────
t = torch.arange(seq_len, device=device) / sample_rate
freq = 440.0  # A4 note
audio_wave = torch.sin(2 * torch.pi * freq * t) + 0.1 * torch.randn_like(t)
audio_wave = audio_wave.unsqueeze(0).repeat(batch_size, 1)  # batch of waveforms

# Expand to embedding space for fusion
...
# Reshape for visualization (batch of waveforms)
num_vis = min(8, batch_size)  # show first 8 examples
original_waveforms = real_data[:num_vis, :, 0].cpu().numpy()  # take first channel
recon_waveforms = reconstructed[:num_vis, 0].cpu().numpy()  # reconstructed scalar proxy

fig, axes = plt.subplots(2, num_vis, figsize=(num_vis * 2, 4), sharex=True, sharey=True)
for i in range(num_vis):
    axes[0, i].plot(original_waveforms[i])
    axes[0, i].set_title(f"Masked Audio #{i+1}")
    axes[0, i].axis('off')

    axes[1, i].plot(recon_waveforms[i])
    axes[1, i].set_title(f"Reconstructed #{i+1}")
    axes[1, i].axis('off')

plt.suptitle("Original Masked vs Triality Reconstructed Waveforms")
plt.tight_layout()
plt.savefig("audio_fusion_waveform_comparison.png", dpi=300, bbox_inches='tight')
plt.show()

print("Waveform visualization saved as audio_fusion_waveform_comparison.png")
