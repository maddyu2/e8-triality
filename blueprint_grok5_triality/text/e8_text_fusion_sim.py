...
# ────────────────────────────────────────────────
# Sparse text proxies (40–70% masking, text coherence 0.85–1.0)
missing_text = torch.linspace(0.4, 0.7, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)
text_emb = torch.linspace(0.5, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)  # proxy
text_coherence = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

text_sym = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

real_data = torch.cat([missing_text, text_emb, text_coherence, text_sym], dim=-1)\
             .repeat(1, 1, dim // 4) * torch.randn(batch_size, seq_len, dim, device=device) * noise_scale

# Apply masking
mask = torch.rand_like(real_data) < missing_text
real_data[mask] = 0
...
class TextCycleBlock(nn.Module):
    def __init__(self):
        super().__init__()
...
class E8TextFusion(nn.Module):
    def __init__(self, depth=256, use_triality=True):
        ...
        self.cycle_block = TextCycleBlock()
...
# Training – optimized
# ────────────────────────────────────────────────
model = E8TextFusion(use_triality=True).to(device)
...
# ────────────────────────────────────────────────
model_ablation = E8TextFusion(use_triality=False).to(device)
...
plt.tight_layout()
plt.savefig("text_fusion_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as text_fusion_ablation_precision_entropy.png")
