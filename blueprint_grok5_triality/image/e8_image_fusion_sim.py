...
# ────────────────────────────────────────────────
# Sparse image proxies (40–70% masking, image coherence 0.85–1.0)
...
image_coherence = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

image_sym = torch.linspace(0.85, 1.0, batch_size * seq_len, device=device).view(batch_size, seq_len, 1)

real_data = torch.cat([missing_image, image_emb, image_coherence, image_sym], dim=-1)\
             .repeat(1, 1, dim // 4) * torch.randn(batch_size, seq_len, dim, device=device) * noise_scale

# Apply masking
mask = torch.rand_like(real_data) < missing_image
real_data[mask] = 0
...
plt.tight_layout()
plt.savefig("image_fusion_ablation_precision_entropy.png", dpi=300, bbox_inches='tight')
plt.show()

print("Plots saved as image_fusion_ablation_precision_entropy.png")
